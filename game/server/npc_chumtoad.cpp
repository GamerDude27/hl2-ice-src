...
//ConVar	sk_chumtoad_fast_health( "sk_chumtoad_fast_health","0");
//ConVar	sk_chumtoad_poison_health( "sk_chumtoad_poison_health","0");
ConVar	sk_chumtoad_melee_dmg( "sk_chumtoad_melee_dmg","0");
//ConVar	sk_chumtoad_poison_npc_damage( "sk_chumtoad_poison_npc_damage", "0" );

BEGIN_DATADESC( CBaseChumtoad )
...
//-----------------------------------------------------------------------------
void CBaseChumtoad::ChumtoadInit()
{
...
//-----------------------------------------------------------------------------
void CBaseChumtoad::Precache( void )
{
...
//-----------------------------------------------------------------------------
void CBaseChumtoad::CrawlFromCanister()
{
...
// Input  : NewActivity - 
//-----------------------------------------------------------------------------
void CBaseChumtoad::OnChangeActivity( Activity NewActivity )
{
...
// Output : 
//-----------------------------------------------------------------------------
Class_T	CBaseChumtoad::Classify( void )
{
	if( m_bHidden )
	{
...
// Output : Vector
//-----------------------------------------------------------------------------
Vector CBaseChumtoad::BodyTarget( const Vector &posSrc, bool bNoisy ) 
{ 
	Vector vecResult;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CBaseChumtoad::GetAutoAimRadius()
{ 
...
// Output : float
//-----------------------------------------------------------------------------
float CBaseChumtoad::MaxYawSpeed( void )
{
...
bool CBaseChumtoad::IsFirmlyOnGround()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CBaseChumtoad::MoveOrigin( const Vector &vecDelta )
{
...
// Input  : vecPos - 
//-----------------------------------------------------------------------------
void CBaseChumtoad::ThrowAt( const Vector &vecPos )
{
...
// Input  : vecPos - 
//-----------------------------------------------------------------------------
void CBaseChumtoad::JumpToBurrowHint( CAI_Hint *pHint )
{
...
// Input  : vecVel - 
//-----------------------------------------------------------------------------
void CBaseChumtoad::Leap( const Vector &vecVel )
{
	SetTouch( &CBaseChumtoad::LeapTouch );
	...
	m_bMidJump = true;
	SetThink( &CBaseChumtoad::ThrowThink );
	SetNextThink( gpGlobals->curtime );
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseChumtoad::ThrowThink( void )
{
	...
	if( GetFlags() & FL_ONGROUND )
	{
		SetThink( &CBaseChumtoad::CallNPCThink );
...
//			bThrown - 
//-----------------------------------------------------------------------------
void CBaseChumtoad::JumpAttack( bool bRandomJump, const Vector &vecPos, bool bThrown )
{
	Vector vecJumpVel;
	if ( !bRandomJump )
	{
...
// Input  : *pEvent - 
//-----------------------------------------------------------------------------
void CBaseChumtoad::HandleAnimEvent( animevent_t *pEvent )
{
...
//-----------------------------------------------------------------------------
void CBaseChumtoad::SetBurrowed( bool bBurrowed )
{
	if ( bBurrowed )
	{
		AddEffects( EF_NODRAW );
...
// Input  : *pTask - 
//-----------------------------------------------------------------------------
void CBaseChumtoad::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
//-----------------------------------------------------------------------------
bool CBaseChumtoad::HasHeadroom()
{
	trace_t tr;
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CBaseChumtoad::LeapTouch( CBaseEntity *pOther )
{
	m_bMidJump = false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CBaseChumtoad::CalcDamageInfo( CTakeDamageInfo *pInfo )
{
...
//-----------------------------------------------------------------------------
void CBaseChumtoad::TouchDamage( CBaseEntity *pOther )
{
	CTakeDamageInfo info;
	CalcDamageInfo( &info );
...
//---------------------------------------------------------
//---------------------------------------------------------
void CBaseChumtoad::GatherConditions( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseChumtoad::PrescheduleThink( void )
{
...
//-----------------------------------------------------------------------------
void CBaseChumtoad::EliminateRollAndPitch()
{
	QAngle angles = GetAbsAngles();
	...
	SetContextThink( &CBaseChumtoad::EliminateRollAndPitch, gpGlobals->curtime + TICK_INTERVAL, s_pPitchContext );
}
...
//-----------------------------------------------------------------------------
void CBaseChumtoad::BeginClimbFromCanister()
{
...
void CBaseChumtoad::JumpFromCanister()
{
	Assert( GetMoveParent() );
...
void CBaseChumtoad::DropFromCeiling( void )
{
...
//-----------------------------------------------------------------------------
void CBaseChumtoad::PlayerHasIlluminatedNPC( CBasePlayer *pPlayer, float flDot )
{
	if ( flDot < 0.97387f )
		return;
...
bool CBaseChumtoad::CanBeAnEnemyOf( CBaseEntity *pEnemy )
{
#ifdef HL2_EPISODIC
	if ( IsHangingFromCeiling() )
		return false;
...
// Input  : pTask - 
//-----------------------------------------------------------------------------
void CBaseChumtoad::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	case TASK_CHUMTOAD_WAIT_FOR_BARNACLE_KILL:
...
// Output :
//-----------------------------------------------------------------------------
float CBaseChumtoad::InnateRange1MinRange( void )
{
	return CHUMTOAD_MIN_JUMP_DIST;
}

float CBaseChumtoad::InnateRange1MaxRange( void )
{
	return CHUMTOAD_MAX_JUMP_DIST;
}

int CBaseChumtoad::RangeAttack1Conditions( float flDot, float flDist )
{
...
// Output :
//------------------------------------------------------------------------------
bool CBaseChumtoad::CorpseGib( const CTakeDamageInfo &info )
{
...
// Input  :
//------------------------------------------------------------------------------
void CBaseChumtoad::Touch( CBaseEntity *pOther )
{ 
...
// Output : 
//-----------------------------------------------------------------------------
int CBaseChumtoad::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CBaseChumtoad::ClampRagdollForce( const Vector &vecForceIn, Vector *vecForceOut )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CBaseChumtoad::Event_Killed( const CTakeDamageInfo &info )
{
...
// Input  : Type - 
//-----------------------------------------------------------------------------
int CBaseChumtoad::TranslateSchedule( int scheduleType )
{
	switch( scheduleType )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
int CBaseChumtoad::SelectSchedule( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CBaseChumtoad::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
...
//			*ptr - 
//-----------------------------------------------------------------------------
void CBaseChumtoad::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CBaseChumtoad::Ignite( float flFlameLifetime, bool bNPCOnly, float flSize, bool bCalledByLevelDesigner )
{
...
//-----------------------------------------------------------------------------
bool CBaseChumtoad::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
	...
	else if (interactionType ==	g_interactionVortigauntStompHit)
	{
		...
		CBaseChumtoad *pEntity = (CBaseChumtoad*) CreateEntityByName( "npc_chumtoad" );
		pEntity->Spawn();
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CBaseChumtoad::FValidateHintType( CAI_Hint *pHint )
{
	return true;
}
...
// Input  : &origin - 
//-----------------------------------------------------------------------------
void CBaseChumtoad::ClearBurrowPoint( const Vector &origin )
{
...
//-----------------------------------------------------------------------------
bool CBaseChumtoad::ValidBurrowPoint( const Vector &point )
{
	trace_t	tr;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseChumtoad::GrabHintNode( CAI_Hint *pHint )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CBaseChumtoad::FindBurrow( const Vector &origin, float distance, bool excludeNear )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseChumtoad::Burrow( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseChumtoad::Unburrow( void )
{
...
//-----------------------------------------------------------------------------
void CBaseChumtoad::InputUnburrow( inputdata_t &inputdata )
{
	if ( IsAlive() == false )
		return;
...
//-----------------------------------------------------------------------------
void CBaseChumtoad::InputBurrow( inputdata_t &inputdata )
{
	if ( IsAlive() == false )
		return;
...
//-----------------------------------------------------------------------------
void CBaseChumtoad::InputBurrowImmediate( inputdata_t &inputdata )
{
	if ( IsAlive() == false )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseChumtoad::InputStartHangingFromCeiling( inputdata_t &inputdata )
{
	if ( IsAlive() == false )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseChumtoad::InputDropFromCeiling( inputdata_t &inputdata )
{
	if ( IsAlive() == false )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseChumtoad::CreateDust( bool placeDecal )
{
	trace_t	tr;
	AI_TraceLine( GetAbsOrigin()+Vector(0,0,1), GetAbsOrigin()-Vector(0,0,64), MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr );
	...
	if ( tr.fraction < 1.0f )
	{
		...
		if ( ( (char) pdata->game.material == CHAR_TEX_CONCRETE ) || ( (char) pdata->game.material == CHAR_TEX_DIRT ) )
		{
			...
			//DispatchEffect( "chumtoaddust", data );
...
// Purpose: 
//-----------------------------------------------------------------------------
Activity CChumtoad::NPC_TranslateActivity( Activity eNewActivity )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CChumtoad::PainSound( const CTakeDamageInfo &info )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CChumtoad::DeathSound( const CTakeDamageInfo &info )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CChumtoad::TelegraphSound( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CChumtoad::AttackSound( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CChumtoad::BiteSound( void )
{
	EmitSound( "NPC_HeadCrab.Bite" );
}


LINK_ENTITY_TO_CLASS( npc_chumtoad, CChumtoad );
...
// Output : 
//-----------------------------------------------------------------------------
float CChumtoad::MaxYawSpeed ( void )
{
	switch ( GetActivity() )
	{
	case ACT_IDLE:			
...
//-----------------------------------------------------------------------------
void CBaseChumtoad::BuildScheduleTestBits( void )
{
...
AI_BEGIN_CUSTOM_NPC( npc_chumtoad, CBaseChumtoad )
