...
public:
	...
	{
		static const Vector cone = (0,0,0);
		return cone;
	}
...
void CWeaponAtomSmasher::SecondaryAttack( void )
{
	...
	if(m_flNextChargeTime < gpGlobals->curtime)
	{
		...
		else if(m_nChargeType == 2)
		{
			...
			//m_flNextChargeTime = gpGlobals->curtime + SequenceDuration( ACT_VM_PULLBACK_HIGH );
...
void CWeaponAtomSmasher::AttackCharge( int ChargeType )
{
	...
	if(ChargeType == 1)
	{
		//Warning("CT = 1, m_iPlayerDamage = 20, force  = 100 \n");
		info.m_iDamage = 20;
	...
	{
		//Warning("CT = 2, m_iPlayerDamage = 80, force  = 300 \n");
		info.m_iDamage = 80;
	...
	{
		//Warning("CT = 3, m_iPlayerDamage = 220, force  = 1000 \n");
		info.m_iDamage = 220;
...
void CWeaponAtomSmasher::DoImpactEffect( trace_t &tr, int nDamageType )
{
	...
	if (!(tr.surface.flags & SURF_SKY))
	{
		...
		if(!( enginetrace->GetPointContents( tr.endpos ) & (CONTENTS_WATER|CONTENTS_SLIME) ))
		{
			...
			// old temp glow sprite ( had clipping issues)
			//te->GlowSprite( filter, 0.0, &newEnd, m_nGlowSpriteIndex , 1, 1, 100 );
