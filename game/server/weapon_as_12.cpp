...
#define WALKSPEED_SUIT 100
#define WALKSPEED_UNSUIT 90

class CWeaponAS12 : public CBaseHLCombatWeapon
{
	DECLARE_DATADESC();
public:
	DECLARE_CLASS( CWeaponAS12, CBaseHLCombatWeapon );

	DECLARE_SERVERCLASS();

private:
	bool	m_bWasIronBeforeReload;  // was in ironsight before reload
...
public:
	...
	virtual const Vector& GetBulletSpread( void )
	{
		...
		static Vector Ironcone = VECTOR_CONE_6DEGREES;
...
IMPLEMENT_SERVERCLASS_ST(CWeaponAS12, DT_WeaponAS12)
END_SEND_TABLE()

LINK_ENTITY_TO_CLASS( weapon_as_12, CWeaponAS12 );
PRECACHE_WEAPON_REGISTER(weapon_as_12);

BEGIN_DATADESC( CWeaponAS12 )
	DEFINE_FIELD( m_bWasIronBeforeReload, FIELD_BOOLEAN ),
...
acttable_t	CWeaponAS12::m_acttable[] = 
{
...
IMPLEMENT_ACTTABLE(CWeaponAS12);

void CWeaponAS12::Precache( void )
{
...
// Input  : *pOperator - 
//-----------------------------------------------------------------------------
void CWeaponAS12::FireNPCPrimaryAttack( CBaseCombatCharacter *pOperator, bool bUseWeaponAngles )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponAS12::Operator_ForceNPCFire( CBaseCombatCharacter *pOperator, bool bSecondary )
{
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponAS12::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
	switch( pEvent->event )
	{
...
//-----------------------------------------------------------------------------
float CWeaponAS12::GetMinRestTime()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CWeaponAS12::GetMaxRestTime()
{
	if( hl2_episodic.GetBool() && GetOwner() && GetOwner()->Classify() == CLASS_COMBINE )
	{
		return 1.5f;
...
// Output :
//-----------------------------------------------------------------------------
bool CWeaponAS12::StartReload( void )
{
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponAS12::FinishReload( void )
{
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponAS12::FillClip( void )
{
	CBaseCombatCharacter *pOwner  = GetOwner();
...
//
//-----------------------------------------------------------------------------
void CWeaponAS12::PrimaryAttack( void )
{
	...
	// If we are ironsighted we do a tighter grip fire animation, for less kickback
	CBaseCombatWeapon *pWeapon = GetOwner()->GetActiveWeapon();
	if( pWeapon != NULL ){
		if( pWeapon->IsIronsighted())
			SendWeaponAnim( ACT_VM_HITCENTER ); // using ACT_VM_HITCENTER as the animation name in qc
		else
...
//
//-----------------------------------------------------------------------------
void CWeaponAS12::SecondaryAttack( void )
{
	// Only the player fires this way so we can cast
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
...
//-----------------------------------------------------------------------------
void CWeaponAS12::ItemPostFrame( void )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
	if (!pOwner)
	{
	...
	// check if play is ironsight
	CBaseCombatWeapon *pWeapon = pOwner->GetActiveWeapon();
	if( pWeapon->IsIronsighted() ){

		// if you are out of ammo, dis-ironsight cause its annoying to get stuck like that
		if( !pWeapon->HasAmmo() )
		{
			pWeapon->ToggleIronsights();
		}

		// restric running in ironsight
		if( pOwner->IsSuitEquipped() )
		{
			pOwner->SetMaxSpeed( WALKSPEED_SUIT );
		}
		else
		{
			pOwner->SetMaxSpeed( WALKSPEED_UNSUIT );
		}

		// if I'm in ironsight, un-ironsight me for reload, but save the setting to comeback to when done reloading
		if (m_bInReload)
		{
			m_bWasIronBeforeReload = true;
			pWeapon->ToggleIronsights();
		}
	}
	...
	{	
		if( m_bWasIronBeforeReload )
		{
			m_bWasIronBeforeReload = false;
			CBaseCombatWeapon *pWeapon = pOwner->GetActiveWeapon();
			if( pWeapon != NULL ){
				if(!pWeapon->IsIronsighted())
					pWeapon->ToggleIronsights();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponAS12::ItemHolsterFrame( void )
{
	// Must be player held
...
//==================================================
/*
void CWeaponAS12::WeaponIdle( void )
{
	//Only the player fires this way so we can cast
