...
void CFastZombie::Spawn( void )
{
	...
	m_fIsTorso = 0; // change from one line of code to two
	m_fIsHeadless = false; // see!
...
#ifdef HL2_EPISODIC
	SetBloodColor( BLOOD_COLOR_ZOMBIE ); // ICEMOD switched out BLOOD_COLOR_ZOMBIE
#else
	SetBloodColor( BLOOD_COLOR_YELLOW ); // ICE changed blood to red, fuck BLOOD_COLOR_YELLOW
#endif // HL2_EPISODIC
	...
	if ( m_fIsTorso == 1 || m_fIsTorso == 2 || m_fIsTorso == 3 ) // torsos, penguins and baby penguins
	{
...
int CFastZombie::TranslateSchedule( int scheduleType )
{
	...
	case SCHED_MELEE_ATTACK1:
		if ( m_fIsTorso == 1 || m_fIsTorso == 2 || m_fIsTorso == 3 ) // torsos, penguins, baby penguins? I guess..
		{
...
{
	if( m_fIsTorso == 1 || m_fIsTorso == 2 || m_fIsTorso == 3) // torsos, penguins, babypenguins
	{
		// Already split.
		return false;
	}
