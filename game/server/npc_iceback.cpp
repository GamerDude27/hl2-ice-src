...
#ifndef HL2_EPISODIC
	void CheckFlinches() {} // Iceback has custom flinch code
#endif // HL2_EPISODIC
...
#else
	SetBloodColor( DONT_BLEED ); // ICE, GREEN to RED
#endif // HL2_EPISODIC
...
Activity CIceback::NPC_TranslateActivity( Activity newActivity )
{
	...
	//if ( newActivity == ACT_ICEBACK_TANTRUM ) // not a big iceback
		//return ACT_IDLE;

	//if ((m_fIsTorso != 0) && ( newActivity == ACT_ICEBACK_TANTRUM ) ) // not a big iceback
		//return ACT_IDLE;
...
int CIceback::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	if ( inputInfo.GetDamageType() & DMG_BUCKSHOT )
	{
		if(inputInfo.GetDamage() > (m_iMaxHealth/3) )
		{
			...
			// The reason for making sure we did at least 1/3rd of the iceback's max health
			// is so the iceback doesn't flinch every time the odd shotgun pellet hits them,
			// and so the maximum number of times you'll see a iceback flinch like this is 2.(sjb)
