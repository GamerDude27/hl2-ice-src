...
public:
	...
	// ICEMOD
	bool	IsHelmetEquipped() const	{ return m_Local.m_bWearingHelmet; }
	int		GetHelmetDamageAlpha() const { return m_Local.m_iHelmetDamageAlpha; }
	void	SetHelmetDamageAlpha(int DmgAlpha) { if((DmgAlpha < 255) && (DmgAlpha >= 0))m_Local.m_iHelmetDamageAlpha = DmgAlpha; }

	//void	SetHelmetDamage( int Dmg );
	//int		GetHelmetDamage( void ){ return m_Local.m_iHelmetDamageAlpha; } //worked without m_Local.
	int		GetHelmetBloodColor( void ){ return m_Local.m_iHelmetBloodColor; } //worked without m_Local.
	void	SetHelmetBloodColor(int newColor ) { if(newColor >= 2 ) m_Local.m_iHelmetBloodColor = 0; else m_Local.m_iHelmetBloodColor = newColor; }

	int		GetHelmetBloodAlpha( void ){ return m_Local.m_iHelmetBloodAlpha; } //worked without m_Local.
	//void	SetHelmetBloodAlpha(int BldAlpha) { if((BldAlpha < 255) && (BldAlpha >= 0))m_Local.m_iHelmetBloodAlpha = BldAlpha; }
	
	void	SetHelmetBloodAlpha(int BldAlpha) 
	{ 
		if((m_Local.m_iHelmetBloodAlpha + BldAlpha) < 25)
		{
			m_Local.m_iHelmetBloodAlpha += BldAlpha;
		}
		else
		{
			if(m_Local.m_iHelmetBloodAlpha+1 < 25)
				m_Local.m_iHelmetBloodAlpha++;
		}
		Msg("player.h called it");
	}

	/*void	AddHelmetBloodSplatter(int blcolor, int alpha ) 
	{ 
		if(blcolor >= 2 )
			blcolor = 0;
			...
			m_Local.m_iHelmetBloodColor = newColor; 
	}*/
	...
	virtual void EquipHelmet( void ); // ICEMOD we add a helmet
	virtual void RemoveHelmet( void ); // ICEMOD we remove a helmet
...
public:
	...
	void		SetWearingHelmet( void );
	void		SetWearingHelmet( bool oPtion );

private:
	// helmet hints
	bool		m_bDidFreezHelmetHint;
	int			m_iNumberOfHelmetEntries;
	int			m_iNonSuppressedDamage;
...
public:
	bool					m_bPlayerWearingHelmet; // ICEMOD helmet on or off?
...
protected:
	...
	// Helmet Effects
	//int						m_iHelmetDamageAlpha;
	float					m_flHelmetDamageAlphaTime;

	float					m_flHelmetBloodAlphaTime;
	//int						m_iHelmetBloodAlpha;
	//int						m_iHelmetBloodColor;
