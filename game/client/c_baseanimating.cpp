...
void C_BaseAnimating::ProcessMuzzleFlashEvent()
{
	...
	if ( muzzleflash_light.GetBool() )
	{
		...
		{
			/* ICEMOD replace with dlights for dynamic effect
			Vector vAttachment;
			QAngle dummyAngles;
			...
			*/
			...
			ICEMOD - 
			JASON feb 19 2014: 
			Pasted Code below (from alyx code) to maybe help skip the ugly 
			flashing detail prop glaciers later when I find 
			out what the level will be
			called. Hope it works!
			...
			*/
			...
			// 252, 238, 128 for a realistic washed out yellow
			dl->color.r = 252;
			dl->color.g = 238;
			dl->color.b = 128;
...
//			attachments[] - 
//-----------------------------------------------------------------------------
void C_BaseAnimating::SnowFootPrint( C_BaseAnimating *pEnt, bool IsLeft, bool IsCombine )
{
	trace_t tr;
	Vector traceStart;
	QAngle angles;
		...
		if( psurf )
		{
			...
			if ( psurf->game.material != 'X' )
			{
				...
				if (footprintDecal != -1)
				{
					...
					VectorMA( pEnt->GetAbsOrigin(), 
						IsLeft ? -6 : 6, // 12 = half width
						right, hipOrigin );
					...
					if(!IsCombine){
						fpindex = decalsystem->GetDecalIndexForName( "FootPrintRebel" );
					}
	...
	case CL_EVENT_FOOTSTEP_LEFT:
		{
			...
			if(FStrEq(options,"NPC_CombineS") || FStrEq(options,"NPC_MetroPolice")) // ICEMOD anymore to add?
			{
				SnowFootPrint( this, true, true );
			} 
			else
			{
				SnowFootPrint( this, true, false );
			}
#endif
		}
		break;
	...
	case CL_EVENT_FOOTSTEP_RIGHT:
		{
			...
			if(FStrEq(options,"NPC_CombineS") || FStrEq(options,"NPC_MetroPolice"))
			{
				SnowFootPrint( this, true, true );
			} 
			else
	...
	//ICE Eject Brass 2
		...
		if ( m_Attachments.Count() > 0 )
		{
			if ( MainViewOrigin().DistToSqr( GetAbsOrigin() ) < (256 * 256) )
			{
				...
				if( GetAttachment( 4, attachOrigin, attachAngles ) ) // changed to 3, cause in the def it is: num - 1, so it will be 2, I think...
				{
					tempents->EjectBrass( attachOrigin, attachAngles, GetAbsAngles(), atoi( options ) );
				}
			}
