...
/* for dragon skin
static CTextureReference s_pDragonTexture;
ITexture *GetDragonTexture( void )
{ 
	if ( !s_pDragonTexture )
	{
		s_pDragonTexture.Init( materials->FindTexture( "dragon_sheet", TEXTURE_GROUP_RENDER_TARGET ) );
		Assert( !IsErrorTexture( s_pDragonTexture ) );
		AddReleaseFunc();
	}
	return s_pDragonTexture;
}*/
