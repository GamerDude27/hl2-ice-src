...
//ConVar sk_voloxelican_spit_dmg( "sk_voloxelican_spit_dmg","0");
	...
	if(IsFlying())
	{
		if( gpGlobals->curtime >= m_flNextAttack )
		{
				...
				(m_flEnemyDist < (UniqueSpitDistanceFar() + 100)) )
			{
				...
				{
					trace_t tr;
					AI_TraceLine ( vecMouthPos, (pEnemy->GetAbsOrigin() - 150), MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr );
...
void CNPC_Voloxelican::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_VOLOXELICAN_TAKEOFF )
	{
		if ( GetEnemy() != NULL)
		{
			Msg("AE_VOLOXELICAN_TAKEOFF: I have enemy\n");
			Vector vEnemyPos = GetEnemy()->GetAbsOrigin();
			Takeoff( vEnemyPos );
		}
		else if ( GetNavigator()->GetPath()->GetCurWaypoint() )
		{
			Msg("AE_VOLOXELICAN_TAKEOFF: I have no enemy, but a location.\n");
	...
	{
		Msg("AE_VOLOXELICAN_LANDED: I can walk around now.\n");
	...
	if ( IsFlying() && !m_bLanding)
	{
		...
		if ( GetNavigator()->GetPath()->GetCurWaypoint() )
		{
			...
			else
			{
				...
				//SetSchedule( SCHED_VOLOXELICAN_FLY );
			}

		}
		else if ( !GetTask() || GetTask()->iTask == TASK_WAIT_FOR_MOVEMENT )
		{
			//SetSchedule( SCHED_VOLOXELICAN_IDLE_FLY );
			//MoveVoloxelicanFly( flInterval );
...
void CNPC_Voloxelican::MoveVoloxelicanFly( float flInterval )
{
	...
	if( m_nCurrentInAirState == InAirState_RangeAttacking)
	{
		...
		//QAngle vFlyDirection = GetAbsVelocity();
		...
		// ---------------------------------------------------------
		if ((m_flLastDamageTime + 2) > gpGlobals->curtime) // 2 = SCANNER_EVADE_TIME
		{
			vFlyDirection = vFlyDirection + VelocityToEvade(GetEnemyCombatCharacterPointer());
			SetAbsVelocity(vFlyDirection); // JASON testing
		}
	...
	else if ( m_nCurrentInAirState == InAirState_MoveToLanding )
	{
		...
		if(	!m_bLanding	)
		{
		//Vector vTargetPos = GetEnemyLKP(); // need landing spot

			Vector vTargetPos = GetEnemyLKP();
			//float goalDist = ( UniqueSpitDistanceClose() + ( ( UniqueSpitDistanceFar() - UniqueSpitDistanceClose() ) / 2 ) );
			Vector idealPos = IdealGoalForMovement( vTargetPos, GetAbsOrigin(), UniqueSpitDistanceClose(), 0 );
			//IdealGoalForMovement( vTargetPos, GetAbsOrigin(), goalDist, UniqueSpitDistanceClose() );
		...
		{
			SetFlyingState(FlyState_Walking);
			//SetActivity((Activity)ACT_VOLOXELICAN_LAND);
			Msg("landing complete\n");
			TaskMovementComplete();
		}
	...
	else
	{
		...
		// jason roll is too much.. drop it down
		float flRoll = DotProduct( vRight, vecMoveDir ) * 10;
		flRoll = clamp( flRoll, -10, 10 );
	...
	if ( tr.DidHitWorld() )
	{
		...
		if ( ( GetFlags() & FL_ONGROUND ) ) //( tr.fraction != 1.0f ) ||
		{
			...
			if ( GetSequence() != sequence )
			{
				...
				m_bLanding = true;
				Msg("SetFlyingState( FlyState_Falling );\n");
...
void CNPC_Voloxelican::MoveToTarget( float flInterval, const Vector &vecMoveTarget )
{
	...
	//vecCurrentDir = GetCurrentVelocity(); // not physics flyer, we cant grab Velocity - jason
	...
	//QAngle RotateZ = GetAbsAngles();
	//RotateZ[YAW] += 1;
	//SetAbsAngles(RotateZ);

	/* calc relative banking targets
	Vector forward, right, up;
	GetVectors( &forward, &right, &up );

	QAngle m_vCurrentBanking = GetAbsAngles();

	m_vCurrentBanking[PITCH] = targetDir.x;
	m_vCurrentBanking[ROLL]	 = 50.0f * DotProduct( right, targetDir ); //120 *

	float speedPerc = SimpleSplineRemapVal( GetAbsVelocity().Length(), 0.0f, VOLOXELICAN_AIRSPEED, 0.0f, 1.0f );

	speedPerc = clamp( speedPerc, 0.0f, 1.0f );

	m_vCurrentBanking *= speedPerc;

	m_vCurrentBanking[YAW]	 = 0.0;
	QAngle IwantYaw = GetAbsAngles();
	IwantYaw[PITCH] = m_vCurrentBanking[PITCH];
	IwantYaw[ROLL] = m_vCurrentBanking[ROLL];
	SetAbsAngles(IwantYaw);*/
...
bool CNPC_Voloxelican::Probe( const Vector &vecMoveDir, float flSpeed, Vector &vecDeflect )
{
	...
	if ( tr.fraction < 1.0f )
	{
		...
		CrossProduct( tr.plane.normal - 90, vecUp, vecDeflect ); //tr.plane.normal
...
void CNPC_Voloxelican::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
...
void CNPC_Voloxelican::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
		...
		//
//		case TASK_VOLOXELICAN_PREPARE_TO_FLY:
//		{
//			SetFlyingState( FlyState_Flying );
//			TaskComplete();
//			break;
//		}
	...
	case TASK_VOLOXELICAN_WAIT_POST_RANGE:
		...
		case TASK_FIND_HINTNODE:
		{
			...
			if ( !GetHintNode() )
			{
				Msg("setting hint node, ");
				SetHintNode(CAI_HintManager::FindHint( this, HINT_CROW_FLYTO_POINT, bits_HINT_NODE_NEAREST | bits_HINT_NODE_USE_GROUP, 10000 ));
				//FindHintRandom( CAI_BaseNPC *pNPC, const Vector &position, const CHintCriteria &hintCriteria )
				//SetHintNode(CAI_HintManager::FindHintRandom( this, GetAbsOrigin(), bits_HINT_NODE_NEAREST | bits_HINT_NODE_USE_GROUP));
			}
...
bool CNPC_Voloxelican::CorpseGib( const CTakeDamageInfo &info )
{
	...
	// TODO: voloxelican gibs?
	//CGib::SpawnSpecificGibs( this, VOLOXELICAN_GIB_COUNT, 300, 400, "models/gibs/voloxelican_gibs.mdl");
...
void CNPC_Voloxelican::Precache( void )
{
	...
	PrecacheScriptSound( "NPC_Stalker.Scream" ); //NPC_Crow.Squawk
	PrecacheScriptSound( "NPC_FastHeadcrab.Attack" );
	PrecacheScriptSound( "NPC_Crow.Gib" );
	PrecacheScriptSound( "NPC_Stalker.Ambient01" ); //NPC_Crow.Idle
	PrecacheScriptSound( "NPC_Stalker.Alert" ); //NPC_Crow.Alert
	PrecacheScriptSound( "NPC_Stalker.Die" ); //NPC_Crow.Die
	PrecacheScriptSound( "NPC_Stalker.Pain" ); //NPC_Crow.Pain
...
AI_BEGIN_CUSTOM_NPC( npc_voloxelican, CNPC_Voloxelican )
	...
	//DECLARE_TASK( TASK_VOLOXELICAN_PREPARE_TO_FLY )
	...
	/*DEFINE_SCHEDULE
	(
		SCHED_VOLOXELICAN_FLY_TO_LANDING,

		"	Tasks"
		//"		TASK_VOLOXELICAN_SET_FLY_LANDING	0"
		"		TASK_SET_ACTIVITY					ACTIVITY:ACT_IDLE"
		"		TASK_WAIT							0.1"
		""
		"	Interrupts"
	...
	(
		...
		//"		TASK_GET_PATH_TO_RANDOM_NODE		2000" // --- TASK_GET_PATH_TO_HINTNODE
		//"		TASK_FACE_IDEAL					0" //
		"		TASK_WAIT_FOR_MOVEMENT			0"
		"		"
		"	Interrupts"
	...
	(
		...
		"	Tasks"
		//"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_VOLOXELICAN_FLY_FAIL"
		"		TASK_STOP_MOVING				0"
		//"		TASK_FACE_IDEAL						0"//
		"		TASK_GET_PATH_TO_RANDOM_NODE		2000" // --- TASK_GET_PATH_TO_HINTNODE
