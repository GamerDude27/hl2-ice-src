								...
								float			flLifetime)
{
	for (int i=0;i<nNumGibs;i++)
	{
		...
		pGib->m_bIsVoloxelican_Spit = false;
		pGib->m_bVolDoOnce = false;
...
// ICEMOD I bet there was another way to do this, but this overloaded method allows
// for gibs to have a body group set, at spawn
//------------------------------------------------------------------------------
// Purpose : Given an .mdl file with gibs and the number of gibs in the file
//			 spawns them in pVictim's bounding box
// Input   :
// Output  :
//------------------------------------------------------------------------------
void CGib::AltSpawnSpecificGibs(	CBaseEntity*	pVictim, 
								int				nNumGibs, 
								float			vMinVelocity, 
								float			vMaxVelocity, 
								const char*		cModelName,
								...
								float			flLifetime)
{
	...
	{
		CGib *pGib = CREATE_ENTITY( CGib, "gib" );
		pGib->Spawn( cModelName, (2 + RandomFloat( 0.5f , 0.9f )) );
		pGib->m_nBody = nBodyGroup;
		pGib->InitGib( pVictim, vMinVelocity, vMaxVelocity );
		//pGib->m_lifeTime = 3;
		pGib->m_bIsVoloxelican_Spit = true; //right now this is ok because only voloxelican spit use this overlaoded method!
		pGib->m_bVolDoOnce = true;
					...
					if( psurf )
					{
						...
						if ( psurf->game.material != 'X' )
						{
							...
							if ( pSporeExplosion )
							{
								...
								pSporeExplosion->SetLifetime( 5 );

								UTIL_BloodDrips( GetAbsOrigin(), dir, BLOOD_COLOR_GREEN, 50 ); //vec3_origin = dir, vecSpot = GetAbsOrigin()

								EmitSound("NPC_Antlion.RunOverByVehicle");

								RadiusDamage( CTakeDamageInfo( this, GetOwnerEntity(), 12, DMG_ACID ), GetAbsOrigin(), 125, CLASS_ANTLION, NULL );

							}
							//UTIL_BloodDrips( vecSpot, vec3_origin, BLOOD_COLOR_GREEN, 50 );
							UTIL_DecalTrace( &tr, "YellowBlood" ); //PaintSplatGreen YellowBlood
						}
					}
				}
			}
...
void CGib::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( pPlayer )
	{
		if(m_bIsVoloxelican_Spit)
		{
			pPlayer->DamageEffect(4, DMG_ACID);
		}
		else
		{
