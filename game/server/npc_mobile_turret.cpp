...
//---------------------------------------------------------
/*---------------------------------------------------------
int CNPC_Mobile_Turret::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
...
int CNPC_Mobile_Turret::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	/*
	if ((info.GetDamageType() & (DMG_SLASH | DMG_CLUB | DMG_DROWN | DMG_GENERIC | DMG_CRUSH)) )
	{

	}
	else
	{
		// Voloxelican attacks are too stong, tone down the scale of poision / acid
		// right now they are classified as CLASS_ANTLION, and altering poision /acid dosnt make a difference.
		if ( info.GetAttacker() && info.GetAttacker()->Classify() == CLASS_ANTLION )
		{
			float flDamage = myInfo.GetDamage();
			flDamage = (myInfo.GetDamage() * 0.05f);
			myInfo.SetDamage( flDamage );
		}
		else
		{
			// scale all damage by everything else down, so that I can live longer.. something is wrong with setting my health
			float flDamage = (myInfo.GetDamage() * 0.05f);
			myInfo.SetDamage( flDamage );
		}
	}*/
	...
	{
		float flDamage = (myInfo.GetDamage() * 0.01f);
		myInfo.SetDamage( flDamage );
	}
...
int CNPC_Mobile_Turret::SelectSchedule()
{
	...
	if(!m_bSeeEnemy)
	{
		...
		/*
		if ( HasCondition ( COND_SEE_PLAYER ) ) // Test for better AI
		{
			Msg("SCHED_LOOK_AROUND \n");
			SetEyeState( MTURRET_EYE_DORMANT );
