...
IMPLEMENT_SERVERCLASS_ST( CNPC_Dragon, DT_Npc_Dragon )
	SendPropTime( SENDINFO( m_flSkinIllume ) ),
	SendPropTime( SENDINFO( m_bIsTorching ) ),
	SendPropTime( SENDINFO( m_iSkinColor ) ),
END_SEND_TABLE()
...
void CNPC_Dragon::Spawn( void )
{
	...
	m_bIsAlert = false;
	// set to min color right away!
	//color32 color = GetRenderColor();
	//color.r = DRAGON_MIN_COLOR;
	//SetRenderColor( color.r, color.r, color.r );
...
void CNPC_Dragon::UpdateHead( void )
{
	...
	if ( ( GetEnemy() != NULL ) && HasCondition( COND_IN_PVS ) )
	{
		...
		if(!m_bParticle)
		{
			...
			//Vector vecShootDir, vecAttachmentOrigin;
			//QAngle vecShootAng;

			//GetAttachment( "mouth", vecAttachmentOrigin, &vecShootDir, NULL, NULL );
			//AngleVectors(vecShootAng, &vecShootDir); //test
			//DispatchParticleEffect( "flamethrower", vecAttachmentOrigin, vecShootAng, this);
		}*/
	}
	else
	{
...
bool CNPC_Dragon::OverrideMove( float flInterval )
{
	...
	if(!m_bIsAlert)
	{
		...
		else
		{
			...
			{
				m_flSkinIllume -= .0005; //DRAGON_ILLUME_REMOVE_RATE;
			} 
			else
			{
				if( m_flSkinIllume != DRAGON_DEAD_ILLUME_MIN)
...
void CNPC_Dragon::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
...
{
	//EmitSound( "NPC_Dragon.Gib" );

	// TODO: dragon gibs?
	//CGib::SpawnSpecificGibs( this, DRAGON_GIB_COUNT, 300, 400, "models/gibs/dragon_gibs.mdl");
...
bool CNPC_Dragon::BecomeRagdollOnClient( const Vector &force )
{
	...
	if( VPhysicsGetObject() )
	{
		...
		speed = min( speed, (DRAGON_RAGDOLL_SPEED_LIMIT * flMass) );
		newForce *= speed;
	}
	*/
...
int CNPC_Dragon::SelectSchedule( void )
{
	...
	if( GetEnemy() != NULL )
	{
		...
		// move to attack
		//if ( HasCondition( COND_DRAGON_ENEMY_TOO_FAR ) )
		//{
			//ClearCondition( COND_DRAGON_ENEMY_TOO_FAR );
		return SCHED_DRAGON_RUN_TO_ATTACK;
		//}
	}
	else
