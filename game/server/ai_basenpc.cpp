...
// ICEMOD AI SNOW FOOTPRINTS
void CAI_BaseNPC::DoSnowFootprint( surfacedata_t *psurface, trace_t ptr, int fpindex )
{
	...
	if ( psurface->game.material != 'X' )
	{
		...
		if (footprintDecal != -1)
		{
			...
			//int fpindex = decalsystem->GetDecalIndexForName( "FootPrintHuman" ); // get the decal by script name
			te->FootprintDecal( filter, 0.0f, &ptr.endpos, &right, 0, //0 = COLLISION_GROUP_NONE?
