...
void CNPC_PoisonZombie::Spawn( void )
{
	...
	m_fIsTorso = 0; // chaged to 2 lines, again!
	m_fIsHeadless = false; // see! seeee!
...
void CNPC_PoisonZombie::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if ( m_fIsTorso == 0 ) // only big zombies
	{
...
void CNPC_PoisonZombie::SetZombieModel( void )
{
	...
	if ( m_fIsTorso != 0) // Im getting lazy, used !=0 instead of the OR's ||.. 
	{
