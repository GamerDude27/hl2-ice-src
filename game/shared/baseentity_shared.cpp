...
{
	// ICEMOD add blood drips on player helmet, if in range of the effect
	...
	if( pPlayer != NULL )
	{
		...
		{
			// Get distance to object and use it as a value.
			Vector vecSegment;
			VectorSubtract( vecSpot, pPlayer->GetAbsOrigin(), vecSegment );
			float flDistance = VectorNormalize( vecSegment );

			// 90 max distance for blood spekels on helmet

			// set the alpha, only if the alpha is starting from 0
			if (pPlayer->m_Local.m_iHelmetBloodAlpha == 0)
			{
				int newColorInt = 0;
				if (bloodColor > -1 && bloodColor != 3 && bloodColor != 5)
				{
					/*
						DONT_BLEED = -1,
					...
					*/
					if (bloodColor == 1 || bloodColor == 2 || bloodColor == 4 || bloodColor == 6)
						newColorInt = 1;

					pPlayer->SetHelmetBloodColor(newColorInt);

					if(flDistance < 90 && flDistance > -1)
					{
						int AmountToAdd = (int)(10 - (flDistance / 10));
							pPlayer->SetHelmetBloodAlpha(AmountToAdd);
					}	
				}
			}
		}
...
void CBaseEntity::FireBullets( const FireBulletsInfo_t &info )
{
	...
	// bullet's final direction can be changed by passing through a portal
		...
		if (tr.fraction != 1.0)
		{
			...
			// ICEMOD, this has been added for the beam weapon to work properly attacking water..
			if (bHitWater && IsPlayer())
			{
				CBasePlayer *pPlayer = static_cast< CBasePlayer * >( this );
				const char* myWeaponType = pPlayer->GetActiveWeapon()->GetClassname();
				if(FStrEq(myWeaponType,"weapon_atomsmasher"))
				{
