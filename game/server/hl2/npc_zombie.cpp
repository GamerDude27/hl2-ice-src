...
// ACT_FLINCH_PHYSICS
#define BIGP_HALFWIDTH 9
#define BABYP_HALFWIDTH 7

ConVar	sk_zombie_health( "sk_zombie_health","0");

ConVar	sk_babypenguin_health( "sk_babypenguin_health","0");
ConVar	sk_penguin_health( "sk_penguin_health","0");
...
public:
	...
	// ICEMOD FOOTPRINT BASE ZOMBIE
	int FootPrintDecal( void ){ int fpdid = decalsystem->GetDecalIndexForName( "FootPrintHuman" ); return fpdid; };
...
LINK_ENTITY_TO_CLASS( npc_penguin, CZombie );
LINK_ENTITY_TO_CLASS( npc_baby_penguin, CZombie );
//LINK_ENTITY_TO_CLASS( npc_iceback, CZombie );
...
void CZombie::Precache( void )
{
	...
	// precache ICE penguin models
	PrecacheModel( "models/Penguins/Penguin.mdl" );
	PrecacheModel( "models/Penguins/BabyPenguin.mdl" );
	// iceback model
	//PrecacheModel( "models/Iceback/iceback.mdl" );

	PrecacheModel( "models/zombie/classic_legs.mdl" );

	PrecacheScriptSound( "Penguin.Calm" );
	PrecacheScriptSound( "Penguin.Crazy" );
	PrecacheScriptSound( "BabyPenguin.Calm" );
	PrecacheScriptSound( "BabyPenguin.Crazy" );
...
void CZombie::Spawn( void )
{
	...
	if( FClassnameIs( this, "npc_zombie" ) )
	{
		...
		m_fIsTorso = 0; // false is now 0
		m_fModelHalfWidth = FOOTPRINT_ZOMBIE;
		m_iHealth = sk_zombie_health.GetFloat();
	}
	else if( FClassnameIs( this, "npc_zombie_torso" ) )
	{
		SetBloodColor( BLOOD_COLOR_ZOMBIE );
		m_fIsTorso = 1; // true is now 1
		m_fModelHalfWidth = FOOTPRINT_ZOMBIE;
		m_iHealth = sk_zombie_health.GetFloat();
	}
	else if( FClassnameIs( this, "npc_penguin" ) )
	{
		SetBloodColor( BLOOD_COLOR_RED );
		m_fIsTorso = 2; // penguin is 2
		m_fModelHalfWidth = FOOTPRINT_PENGUIN;
		m_iHealth = sk_penguin_health.GetFloat();
	}
	else if( FClassnameIs( this, "npc_baby_penguin" ) )
	{
		SetBloodColor( BLOOD_COLOR_RED );
		m_fIsTorso = 3; // babypenguin is 3
		m_fModelHalfWidth = FOOTPRINT_BABYPENGUIN;
		m_iHealth = sk_babypenguin_health.GetFloat();
	}
	//else if( FClassnameIs( this, "npc_iceback" ) )
	//{
		//m_fIsTorso = 4; // babypenguin is 4
	...
	/*
#ifdef HL2_EPISODIC
	SetBloodColor( BLOOD_COLOR_ZOMBIE ); // ICE, ZOMBIE to RED
#else
	SetBloodColor( BLOOD_COLOR_RED ); // ICE, GREEN to RED
#endif // HL2_EPISODIC
	*/
...
void CZombie::PainSound( const CTakeDamageInfo &info )
{
	...
	if(m_fIsTorso == 2) // Penguin Sound
	{
		EmitSound( "Penguin.Crazy" );
	} 
	else if (m_fIsTorso == 3) // Baby Penguin Sound
	{
		EmitSound( "BabyPenguin.Crazy" );
	}
	else // Zombies
	{
...
void CZombie::IdleSound( void )
{
	...
	if(m_fIsTorso == 2) // Penguin Sound
	{
		EmitSound( "Penguin.Calm" );
	} 
	else if (m_fIsTorso == 3) // Baby Penguin Sound
	{
		EmitSound( "BabyPenguin.Calm" );
	}
	else // Zombies
	{
...
void CZombie::SetZombieModel( void )
{
	...
	else if ( m_fIsTorso  == 1 )
	{
		SetModel( "models/zombie/classic.mdl" );
		SetHullType( HULL_HUMAN );
	}
	else if ( m_fIsTorso  == 2 ) // ICE PENGUIN
	{
		SetModel( "models/penguins/penguin.mdl" );
		SetHullType( HULL_HUMAN );
	}
	else if( m_fIsTorso  == 3 )// ICE BABY PENGUIN "3"
	{
		SetModel( "models/penguins/babypenguin.mdl" );
		SetHullType( HULL_HUMAN );
	}
	//else // Iceback
	//{
...
int CZombie::TranslateSchedule( int scheduleType )
{
	...
	if ((m_fIsTorso == 0) && scheduleType == SCHED_FAIL ) // not a torso
...
Activity CZombie::NPC_TranslateActivity( Activity newActivity )
{
	...
	if ((m_fIsTorso != 0) && ( newActivity == ACT_ZOMBIE_TANTRUM ) ) // not a big zombie
		return ACT_IDLE;
...
int CZombie::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	{
		if((m_fIsTorso == 0) && inputInfo.GetDamage() > (m_iMaxHealth/3) )
		{
...
void CZombie::BuildScheduleTestBits( void )
{
	...
	if((m_fIsTorso == 0) && !IsCurSchedule( SCHED_FLINCH_PHYSICS ) && !m_ActBusyBehavior.IsActive() )
	{
