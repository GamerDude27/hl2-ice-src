...
//	MoveHelper()->PlayerSetAnimation( PLAYER_WALK );
	...
	if ( psurface->game.material != 'X' )
	{
		...
		{
			Vector right;
			AngleVectors( GetAbsAngles(), 0, &right, 0 );//pev->angles, 0, &right, 0 );
			...
			int fpindex = decalsystem->GetDecalIndexForName( "FootPrintGordonNH" ); // don't have suit equipped
			if(this->IsSuitEquipped())
			{
				fpindex = decalsystem->GetDecalIndexForName( "FootPrintHuman" ); // get the decal by script name
			}
			te->FootprintDecal( filter, 0.0f, &tr.endpos, &right, 0, //0 = COLLISION_GROUP_NONE?
							   fpindex, 'J' ); // J can be something else if this has any issues

			if(m_IsFootprintOnLeft)
				m_IsFootprintOnLeft = false;
			else
				m_IsFootprintOnLeft = true;
		}
	}
