...
ConVar	g_debug_Hogm( "g_debug_Hogm", "0" );

ConVar Hogm_minforce( "Hogm_minforce", "700" );
ConVar Hogm_maxforce( "Hogm_maxforce", "1500" );
ConVar Hogm_maxmass( "Hogm_maxmass", "250" );
ConVar Hogm_tracelength( "Hogm_tracelength", "250" );
ConVar Hogm_mega_tracelength( "Hogm_mega_tracelength", "850" );
ConVar Hogm_chargetime("Hogm_chargetime", "2" );
ConVar Hogm_pullforce( "Hogm_pullforce", "4000" );
ConVar Hogm_mega_pullforce( "Hogm_mega_pullforce", "8000" );
ConVar Hogm_cone( "Hogm_cone", "0.97" );
ConVar Hogm_ball_cone( "Hogm_ball_cone", "0.997" );
ConVar Hogm_punt_cone( "Hogm_punt_cone", "0.997" );
ConVar player_throwforce2( "player_throwforce2", "1000" );
ConVar Hogm_dmg_glass( "Hogm_dmg_glass", "15" );
ConVar Hogm_right_turrets( "Hogm_right_turrets", "0" );

extern ConVar hl2_normspeed;
extern ConVar hl2_walkspeed;

#define Hogm_BEAM_SPRITE "sprites/orangelight1.vmt"
#define Hogm_GLOW_SPRITE "sprites/glow04_noz.vmt"
#define Hogm_ENDCAP_SPRITE "sprites/orangeflare1.vmt"
#define Hogm_CENTER_GLOW "sprites/orangecore1.vmt"
#define Hogm_BLAST_SPRITE "sprites/orangecore2.vmt"
...
#define	MEGACANNON_MODEL "models/weapons/v_superHogm.mdl"
#define	MEGACANNON_SKIN	1

// -------------------------------------------------------------------------
//  Hogm trace filter to handle special cases
// -------------------------------------------------------------------------

class CTraceFilterHogm : public CTraceFilterSimple
{
public:
	DECLARE_CLASS( CTraceFilterHogm, CTraceFilterSimple );

	CTraceFilterHogm( const IHandleEntity *passentity, int collisionGroup )
...
//-----------------------------------------------------------------------------
// Purpose: Trace a line the special Hogm way!
//-----------------------------------------------------------------------------
void UTIL_HogmTraceLine( const Vector &vecAbsStart, const Vector &vecAbsEnd, CBaseEntity *pTraceOwner, trace_t *pTrace )
{
	...
	CTraceFilterHogm filter( pTraceOwner, COLLISION_GROUP_NONE );
	UTIL_TraceLine( vecAbsStart, vecAbsEnd, (MASK_SHOT|CONTENTS_GRATE), &filter, pTrace );
...
//-----------------------------------------------------------------------------
// Purpose: Trace a hull for the Hogm
//-----------------------------------------------------------------------------
void UTIL_HogmTraceHull( const Vector &vecAbsStart, const Vector &vecAbsEnd, const Vector &vecAbsMins, const Vector &vecAbsMaxs, CBaseEntity *pTraceOwner, trace_t *pTrace )
{
	// Default to HL2 vanilla
	if ( hl2_episodic.GetBool() == false )
	{
...
//-----------------------------------------------------------------------------
IPhysicsObject *GetRagdollChildAtPosition2( CBaseEntity *pTarget, const Vector &position )
{
	// Check for a ragdoll
...
struct game_shadowcontrol_params_t2 : public hlshadowcontrol_params_t
{
	DECLARE_SIMPLE_DATADESC();
};

BEGIN_SIMPLE_DATADESC( game_shadowcontrol_params_t2 )
...
//-----------------------------------------------------------------------------
class CGrabController2 : public IMotionEvent
{
	DECLARE_SIMPLE_DATADESC();

public:

	CGrabController2( void );
	~CGrabController2( void );
	void AttachEntity( CBasePlayer *pPlayer, CBaseEntity *pEntity, IPhysicsObject *pPhys, bool bIsMegaHogm, const Vector &vGrabPosition, bool bUseGrabPosition );
...
private:
	...
	game_shadowcontrol_params_t2	m_shadow;
	...
	friend class CWeaponHogm;
};

BEGIN_SIMPLE_DATADESC( CGrabController2 )
...
CGrabController2::CGrabController2( void )
{
...
void CGrabController2::OnRestore()
{
	if ( m_controller )
	{
...
void CGrabController2::SetTargetPosition( const Vector &target, const QAngle &targetOrientation )
{
...
// Output : float
//-----------------------------------------------------------------------------
float CGrabController2::ComputeError()
{
	if ( m_errorTime <= 0 )
		return 0;
	...
	if ( pAttached )
	{
		...
		{
			DevMsg( "Object attached to Hogm has no physics object\n" );
			DetachEntity( false );
...
void CGrabController2::ComputeMaxSpeed( CBaseEntity *pEntity, IPhysicsObject *pPhysics )
{
	m_shadow.maxSpeed = 1000;
	m_shadow.maxAngular = DEFAULT_MAX_ANGULAR;
	...
	float flMaxMass = Hogm_maxmass.GetFloat();
	if ( flMass <= flMaxMass )
		return;
...
QAngle CGrabController2::TransformAnglesToPlayerSpace( const QAngle &anglesIn, CBasePlayer *pPlayer )
{
...
QAngle CGrabController2::TransformAnglesFromPlayerSpace( const QAngle &anglesIn, CBasePlayer *pPlayer )
{
	if ( m_bIgnoreRelativePitch )
	{
		matrix3x4_t test;
...
void CGrabController2::AttachEntity( CBasePlayer *pPlayer, CBaseEntity *pEntity, IPhysicsObject *pPhys, bool bIsMegaHogm, const Vector &vGrabPosition, bool bUseGrabPosition )
{
...
//	ComputeMaxSpeed( pEntity, pPhys );
	...
	if ( bUseGrabPosition )
	{
		IPhysicsObject *pChild = GetRagdollChildAtPosition2( pEntity, vGrabPosition );
	...
	m_errorTime = bIsMegaHogm ? -1.5f : -1.0f; // 1 seconds until error starts accumulating
	m_error = 0;
	m_contactAmount = 0;
...
void CGrabController2::DetachEntity( bool bClearVelocity )
{
...
IMotionEvent::simresult_e CGrabController2::Simulate( IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime, Vector &linear, AngularImpulse &angular )
{
	game_shadowcontrol_params_t2 shadowParams = m_shadow;
...
float CGrabController2::GetSavedMass( IPhysicsObject *pObject )
{
...
//-----------------------------------------------------------------------------
bool CGrabController2::IsObjectAllowedOverhead( CBaseEntity *pEntity )
{
...
//-----------------------------------------------------------------------------
class CPlayerPickupController2 : public CBaseEntity
{
	DECLARE_DATADESC();
	DECLARE_CLASS( CPlayerPickupController2, CBaseEntity );
public:
	...
	CGrabController2 &GetGrabController() { return m_grabController; }

private:
	CGrabController2		m_grabController;
	CBasePlayer			*m_pPlayer;
};

LINK_ENTITY_TO_CLASS( player_pickup, CPlayerPickupController2 );

//---------------------------------------------------------
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CPlayerPickupController2 )
...
//			*pObject - 
//-----------------------------------------------------------------------------
void CPlayerPickupController2::Init( CBasePlayer *pPlayer, CBaseEntity *pObject )
{
...
// Input  : bool - 
//-----------------------------------------------------------------------------
void CPlayerPickupController2::Shutdown( bool bThrown )
{
...
void CPlayerPickupController2::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
...
//-----------------------------------------------------------------------------
bool CPlayerPickupController2::IsHoldingEntity( CBaseEntity *pEnt )
{
	return ( m_grabController.GetAttached() == pEnt );
}

void PlayerPickupObject2( CBasePlayer *pPlayer, CBaseEntity *pObject )
{
	//Don't pick up if we don't have a phys object.
	if ( pObject->VPhysicsGetObject() == NULL )
		 return;

	CPlayerPickupController2 *pController = (CPlayerPickupController2 *)CBaseEntity::Create( "player_pickup", pObject->GetAbsOrigin(), vec3_angle, pPlayer );
...
BEGIN_SIMPLE_DATADESC( thrown_objects_t2 )
	DEFINE_FIELD( fTimeThrown, FIELD_TIME ),
	DEFINE_FIELD( hEntity,	FIELD_EHANDLE	),
END_DATADESC()

class CWeaponHogm : public CBaseHLCombatWeapon
{
public:
	DECLARE_CLASS( CWeaponHogm, CBaseHLCombatWeapon );
...
protected:
	...
	CBaseEntity *MegaHogmFindObjectInCone( const Vector &vecOrigin, const Vector &vecDir, float flCone, float flCombineBallCone, bool bOnlyCombineBalls );
	...
	inline bool	IsMegaHogm()
	{
		return PlayerHasMegaHogm();
	}
	...
	CUtlVector< thrown_objects_t2 >	m_ThrownEntities;

	float				m_flTimeNextObjectPurge;

protected:
	// Because the Hogm is a leaf class, we can use
	// static variables to store this information, and save some memory.
	// Should the Hogm end up having inheritors, their activate may
	// stomp these numbers, in which case you should make these ordinary members
	// again.
	//
	// The Hogm also caches some pose parameters in SetupGlobalModelData().
	static int m_poseActive;
	static bool m_sbStaticPoseParamsLoaded;
};

bool CWeaponHogm::m_sbStaticPoseParamsLoaded = false;
int CWeaponHogm::m_poseActive = 0;

IMPLEMENT_SERVERCLASS_ST(CWeaponHogm, DT_WeaponHogm)
	SendPropBool( SENDINFO( m_bIsCurrentlyUpgrading ) ),
	SendPropFloat( SENDINFO( m_flTimeForceView ) ),
END_SEND_TABLE()

LINK_ENTITY_TO_CLASS( weapon_Hogm, CWeaponHogm );
PRECACHE_WEAPON_REGISTER( weapon_Hogm );

BEGIN_DATADESC( CWeaponHogm )
	...
	DEFINE_FIELD( m_bHogmState, FIELD_BOOLEAN ),
...
// Purpose: Precache
//-----------------------------------------------------------------------------
void CWeaponHogm::Precache( void )
{
	PrecacheModel( Hogm_BEAM_SPRITE );
	PrecacheModel( Hogm_GLOW_SPRITE );
	PrecacheModel( Hogm_ENDCAP_SPRITE );
	PrecacheModel( Hogm_CENTER_GLOW );
	PrecacheModel( Hogm_BLAST_SPRITE );
	...
	PrecacheScriptSound( "Weapon_Hogm.HoldSound" );
	PrecacheScriptSound( "Weapon_Physgun.Off" );

	PrecacheScriptSound( "Weapon_MegaHogm.DryFire" );
	PrecacheScriptSound( "Weapon_MegaHogm.Launch" );
	PrecacheScriptSound( "Weapon_MegaHogm.Pickup");
	PrecacheScriptSound( "Weapon_MegaHogm.Drop");
	PrecacheScriptSound( "Weapon_MegaHogm.HoldSound");
	PrecacheScriptSound( "Weapon_MegaHogm.ChargeZap");
...
void CWeaponHogm::OnRestore()
{
	...
	// Tracker 8106:  Hogm effects disappear through level transition, so
...
// On Remove
//-----------------------------------------------------------------------------
void CWeaponHogm::UpdateOnRemove(void)
{
	DestroyEffects( );
...
// Sprite scale factor 
//-----------------------------------------------------------------------------
inline float CWeaponHogm::SpriteScaleFactor() 
{
	return IsMegaHogm() ? 1.5f : 1.0f;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponHogm::SetViewModel( void )
{
	if ( !IsMegaHogm() )
	{
...
//-----------------------------------------------------------------------------
void CWeaponHogm::ForceDrop( void )
{
	CloseElements();
	DetachObject();
	StopEffects();
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponHogm::DropIfEntityHeld( CBaseEntity *pTarget )
{
	if ( pTarget == NULL )
		return false;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponHogm::Drop( const Vector &vecVelocity )
{
	ForceDrop();
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CWeaponHogm::CanHolster( void ) 
{ 
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponHogm::Holster( CBaseCombatWeapon *pSwitchingTo )
{
	//Don't holster this weapon if we're holding onto something
	if ( m_bActive )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponHogm::PrimaryFireEffect( void )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
// Punt non-physics
//-----------------------------------------------------------------------------
void CWeaponHogm::PuntNonVPhysics( CBaseEntity *pEntity, const Vector &forward, trace_t &tr )
{
	float flDamage = 1.0f;
	if ( FClassnameIs( pEntity, "func_breakable" ) )
	{
		...
		{
			flDamage = Hogm_dmg_glass.GetFloat();
		}
	}
...
// What happens when the physgun picks up something 
//-----------------------------------------------------------------------------
void CWeaponHogm::Physgun_OnPhysGunPickup( CBaseEntity *pEntity, CBasePlayer *pOwner, PhysGunPickup_t reason )
{
	// If the target is debris, convert it to non-debris
...
// Punt vphysics
//-----------------------------------------------------------------------------
void CWeaponHogm::PuntVPhysics( CBaseEntity *pEntity, const Vector &vecForward, trace_t &tr )
{
	CTakeDamageInfo	info;
	...
	if ( Pickup_OnAttemptPhysGunPickup( pEntity, pOwner, PUNTED_BY_CANNON ) )
	{
		...
		if ( !IsMegaHogm() && !Pickup_ShouldPuntUseLaunchForces( pEntity, PHYSGUN_FORCE_PUNTED ) )
		{
			int i;
...
// Input  : - 
//-----------------------------------------------------------------------------
void CWeaponHogm::ApplyVelocityBasedForce( CBaseEntity *pEntity, const Vector &forward, const Vector &vecHitPos, PhysGunForce_t reason )
{
...
// Punt non-physics
//-----------------------------------------------------------------------------
void CWeaponHogm::PuntRagdoll( CBaseEntity *pEntity, const Vector &vecForward, trace_t &tr )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
// Trace length
//-----------------------------------------------------------------------------
float CWeaponHogm::TraceLength()
{
	if ( !IsMegaHogm() )
	{
		return Hogm_tracelength.GetFloat();
	}
	
	return Hogm_mega_tracelength.GetFloat();
}

//-----------------------------------------------------------------------------
// If there's any special rejection code you need to do per entity then do it here
// This is kinda nasty but I'd hate to move more Hogm related stuff into CBaseEntity
//-----------------------------------------------------------------------------
bool CWeaponHogm::EntityAllowsPunts( CBaseEntity *pEntity )
{
...
//-----------------------------------------------------------------------------
void CWeaponHogm::PrimaryAttack( void )
{
	...
	if( m_bActive )
	{
		...
		if ( pHeld != NULL )
		{
			...
			if ( heldDist > Hogm_tracelength.GetFloat() )
			{
		...
		LaunchObject( forward, Hogm_maxforce.GetFloat() );
	...
	float flPuntDistance = Hogm_tracelength.GetFloat();
	VectorMA( start, flPuntDistance, forward, end );

	trace_t tr;
	UTIL_HogmTraceHull( start, end, -Vector(8,8,8), Vector(8,8,8), pOwner, &tr );
	bool bValid = true;
	...
	if ( !bValid )
	{
		UTIL_HogmTraceLine( start, end, pOwner, &tr );
		if ( tr.fraction == 1 || !tr.m_pEnt || tr.m_pEnt->IsEFlagSet( EFL_NO_PHYSCANNON_INTERACTION ) )
		{
			if( hl2_episodic.GetBool() )
			{
				// Try to find something in a very small cone. 
				CBaseEntity *pObject = FindObjectInCone( start, forward, Hogm_punt_cone.GetFloat() );

				if( pObject )
				{
					// Trace to the object.
					UTIL_HogmTraceLine( start, pObject->WorldSpaceCenter(), pOwner, &tr );
	...
	if ( pEntity->GetMoveType() != MOVETYPE_VPHYSICS )
	{
		...
		if( GetOwner()->IsPlayer() && !IsMegaHogm() )
		{
...
//-----------------------------------------------------------------------------
void CWeaponHogm::SecondaryAttack( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponHogm::WeaponIdle( void )
{
...
// Input  : *pObject - 
//-----------------------------------------------------------------------------
bool CWeaponHogm::AttachObject( CBaseEntity *pObject, const Vector &vPosition )
{
	if ( m_bActive )
		return false;
	...
	m_grabController.AttachEntity( pOwner, pObject, pPhysics, bIsMegaHogm, vPosition, (!bKilledByGrab) );
...
#if defined(HL2_DLL)
	if( Hogm_right_turrets.GetBool() && pObject->ClassMatches("npc_turret_floor") )
	{
...
void CWeaponHogm::FindObjectTrace( CBasePlayer *pPlayer, trace_t *pTraceResult )
{
	Vector forward;
	...
	if( IsMegaHogm() && hl2_episodic.GetBool() )
	{
	...
	UTIL_HogmTraceLine( start, end, pPlayer, pTraceResult );

	// Try again with a hull trace
	if ( !pTraceResult->DidHitNonWorldEntity() )
	{
		UTIL_HogmTraceHull( start, end, -Vector(4,4,4), Vector(4,4,4), pPlayer, pTraceResult );
	}
}


CWeaponHogm::FindObjectResult_t CWeaponHogm::FindObject( void )
{
	...
	if ( !IsMegaHogm() )
	{
		if (!bAttach && !bPull)
		{
			pConeEntity = FindObjectInCone( start, forward, Hogm_cone.GetFloat() );
		}
	}
	else
	{
		pConeEntity = MegaHogmFindObjectInCone( start, forward, 
			Hogm_cone.GetFloat(), Hogm_ball_cone.GetFloat(), bAttach || bPull );
	}
	...
	pullDir *= IsMegaHogm() ? Hogm_mega_pullforce.GetFloat() : Hogm_pullforce.GetFloat();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CBaseEntity *CWeaponHogm::MegaHogmFindObjectInCone( const Vector &vecOrigin, 
   const Vector &vecDir, float flCone, float flCombineBallCone, bool bOnlyCombineBalls )
{
	...
	for( int i = 0 ; i < count ; i++ )
	{
		...
		trace_t tr;
		UTIL_HogmTraceLine( vecOrigin, list[ i ]->WorldSpaceCenter(), GetOwner(), &tr );
		if( tr.m_pEnt == list[ i ] )
		{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CBaseEntity *CWeaponHogm::FindObjectInCone( const Vector &vecOrigin, const Vector &vecDir, float flCone )
{
	// Find the nearest physics-based item in a cone in front of me.
	CBaseEntity *list[256];
	float flNearestDist = Hogm_tracelength.GetFloat() + 1.0; //Use regular distance.
	Vector mins = vecOrigin - Vector( flNearestDist, flNearestDist, flNearestDist );
	Vector maxs = vecOrigin + Vector( flNearestDist, flNearestDist, flNearestDist );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CGrabController2::UpdateObject( CBasePlayer *pPlayer, float flError )
{
 	CBaseEntity *pEntity = GetAttached();
...
void CWeaponHogm::UpdateObject( void )
{
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
	Assert( pPlayer );

	float flError = IsMegaHogm() ? 18 : 12;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWeaponHogm::DetachObject( bool playSound, bool wasLaunched )
{
	if ( m_bActive == false )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponHogm::CheckForTarget( void )
{
...
void CWeaponHogm::BeginUpgrade()
{
	...
	SetContextThink( &CWeaponHogm::WaitForUpgradeThink, gpGlobals->curtime + 6.0f, s_pWaitForUpgradeContext );
...
// Wait until we're done upgrading
//-----------------------------------------------------------------------------
void CWeaponHogm::WaitForUpgradeThink()
{
	...
	{
		SetContextThink( &CWeaponHogm::WaitForUpgradeThink, gpGlobals->curtime + 0.1f, s_pWaitForUpgradeContext );
		return;
	}
	...
	CBaseEntity *pEnt = gEntList.FindEntityByName( NULL, "script_Hogm_upgrade" );
	if ( pEnt )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponHogm::DoEffectIdle( void )
{
	if ( IsEffectActive( EF_NODRAW ) )
	{
		StopEffects();
	...
	if ( m_bHogmState != IsMegaHogm() )
	{
		...
		//This means we just switched to regular Hogm this frame.
		if ( m_bHogmState == false )
		{
	...
	if ( IsMegaHogm() )
	{
		...
		if ( random->RandomInt( 0, 100 ) == 0 && !engine->IsPaused() )
		{
			...
			if ( m_hEndSprites[sprite] != NULL )
			{
				...
				EmitSound( "Weapon_MegaHogm.ChargeZap" );
			}
		}
...
//-----------------------------------------------------------------------------
void CWeaponHogm::ItemBusyFrame( void )
{
	DoEffectIdle();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWeaponHogm::ItemPostFrame()
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
	if ( pOwner == NULL )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
#define Hogm_DANGER_SOUND_RADIUS 128

void CWeaponHogm::LaunchObject( const Vector &vecDir, float flForce )
{
	// FIRE!!!
	if( m_grabController.GetAttached() )
	{
		...
		vecSpot = vecStart + vecDir * Hogm_DANGER_SOUND_RADIUS;

		for( i = Hogm_DANGER_SOUND_RADIUS ; i < iLength ; i += Hogm_DANGER_SOUND_RADIUS )
		{
			CSoundEnt::InsertSound( SOUND_PHYSICS_DANGER, vecSpot, Hogm_DANGER_SOUND_RADIUS, 0.5, pObject );
			vecSpot = vecSpot + ( vecDir * Hogm_DANGER_SOUND_RADIUS );
		}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponHogm::CanPickupObject( CBaseEntity *pTarget )
{
	if ( pTarget == NULL )
		return false;
	...
	if ( !IsMegaHogm() )
	{
		if ( pTarget->VPhysicsIsFlesh( ) )
			return false;
		return CBasePlayer::CanPickupObject( pTarget, Hogm_maxmass.GetFloat(), 0 );
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponHogm::OpenElements( void )
{
	if ( m_bOpen )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponHogm::CloseElements( void )
{
...
// Output : float
//-----------------------------------------------------------------------------
float CWeaponHogm::GetLoadPercentage( void )
{
	float loadWeight = m_grabController.GetLoadWeight();
	loadWeight /= Hogm_maxmass.GetFloat();	
...
// Output : CSoundPatch
//-----------------------------------------------------------------------------
CSoundPatch *CWeaponHogm::GetMotorSound( void )
{
	if ( m_sndMotor == NULL )
	{
		...
		if ( IsMegaHogm() )
		{
			m_sndMotor = (CSoundEnvelopeController::GetController()).SoundCreate( filter, entindex(), CHAN_STATIC, "Weapon_MegaHogm.HoldSound", ATTN_NORM );
		}
		else
		{
			m_sndMotor = (CSoundEnvelopeController::GetController()).SoundCreate( filter, entindex(), CHAN_STATIC, "Weapon_Hogm.HoldSound", ATTN_NORM );
		}
	}
...
// Shuts down sounds
//-----------------------------------------------------------------------------
void CWeaponHogm::StopLoopingSounds()
{
	if ( m_sndMotor != NULL )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponHogm::DestroyEffects( void )
{
	//Turn off main glow
	if ( m_hCenterSprite != NULL )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponHogm::StopEffects( bool stopSound )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponHogm::StartEffects( void )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
	if ( pOwner == NULL )
		return;

	bool bIsMegaCannon = IsMegaHogm();
	...
	for ( i = 0; i < NUM_BEAMS; i++ )
	{
		...
		m_hBeams[i] = CBeam::BeamCreate( 
			bIsMegaCannon ? MEGACANNON_BEAM_SPRITE : Hogm_BEAM_SPRITE, 1.0f );
	...
	for ( i = 0; i < NUM_SPRITES; i++ )
	{
		...
		m_hGlowSprites[i] = CSprite::SpriteCreate( 
			bIsMegaCannon ? MEGACANNON_GLOW_SPRITE : Hogm_GLOW_SPRITE, 
			GetAbsOrigin(), false );
	...
	for ( i = 0; i < 2; i++ )
	{
		if ( m_hEndSprites[i] == NULL )
		{
			...
			m_hEndSprites[i] = CSprite::SpriteCreate( 
				bIsMegaCannon ? MEGACANNON_ENDCAP_SPRITE : Hogm_ENDCAP_SPRITE, 
				GetAbsOrigin(), false );
	...
	if ( m_hCenterSprite == NULL )
	{
		m_hCenterSprite = CSprite::SpriteCreate( 
			bIsMegaCannon ? MEGACANNON_CENTER_GLOW : Hogm_CENTER_GLOW, 
			GetAbsOrigin(), false );
	...
	if ( m_hBlastSprite == NULL )
	{
		m_hBlastSprite = CSprite::SpriteCreate( 
			bIsMegaCannon ? MEGACANNON_BLAST_SPRITE : Hogm_BLAST_SPRITE, 
			GetAbsOrigin(), false );
...
// Closing effects
//-----------------------------------------------------------------------------
void CWeaponHogm::DoEffectClosed( void )
{
	float flScaleFactor = SpriteScaleFactor();
...
// Closing effects
//-----------------------------------------------------------------------------
void CWeaponHogm::DoMegaEffectClosed( void )
{
	float flScaleFactor = SpriteScaleFactor();
...
// Ready effects
//-----------------------------------------------------------------------------
void CWeaponHogm::DoEffectReady( )
{
	float flScaleFactor = SpriteScaleFactor();
...
// Holding effects
//-----------------------------------------------------------------------------
void CWeaponHogm::DoEffectHolding( )
{
	float flScaleFactor = SpriteScaleFactor();
...
// Launch effects
//-----------------------------------------------------------------------------
void CWeaponHogm::DoEffectLaunch( Vector *pos )
{
	Assert( pos );
	if ( pos == NULL )
		return;
	...
	CBeam *pBeam = CBeam::BeamCreate( IsMegaHogm() ? MEGACANNON_BEAM_SPRITE : Hogm_BEAM_SPRITE, 8 );
...
// Input  : *pos - 
//-----------------------------------------------------------------------------
void CWeaponHogm::DoMegaEffectLaunch( Vector *pos )
{
	Assert( pos );
	if ( pos == NULL )
		return;
	...
	CBeam *pBeam = CBeam::BeamCreate( IsMegaHogm() ? MEGACANNON_BEAM_SPRITE : Hogm_BEAM_SPRITE, 0.8 );
	...
	{
		pBeam = CBeam::BeamCreate( IsMegaHogm() ? MEGACANNON_BEAM_SPRITE : Hogm_BEAM_SPRITE, 0.8 );
...
// Holding effects
//-----------------------------------------------------------------------------
void CWeaponHogm::DoMegaEffectHolding( void )
{
	float flScaleFactor = SpriteScaleFactor();
...
// Ready effects
//-----------------------------------------------------------------------------
void CWeaponHogm::DoMegaEffectReady( void )
{
	float flScaleFactor = SpriteScaleFactor();
...
//-----------------------------------------------------------------------------
void CWeaponHogm::DoEffectNone( void )
{
	if ( m_hBlastSprite != NULL )
	{
		// Become small
...
//			*pos - 
//-----------------------------------------------------------------------------
void CWeaponHogm::DoMegaEffect( int effectType, Vector *pos )
{
	switch( effectType )
	{
	case EFFECT_CLOSED:
...
// Input  : effectType - 
//-----------------------------------------------------------------------------
void CWeaponHogm::DoEffect( int effectType, Vector *pos )
{
...
// Output : const char
//-----------------------------------------------------------------------------
const char *CWeaponHogm::GetShootSound( int iIndex ) const
{
	// Just do this normally if we're a normal Hogm
	if ( PlayerHasMegaHogm() == false )
	...
	{
	case EMPTY:
		return "Weapon_MegaHogm.DryFire";
		break;

	case SINGLE:
		return "Weapon_MegaHogm.Launch";
		break;

	case SPECIAL1:
		return "Weapon_MegaHogm.Pickup";
		break;

	case MELEE_MISS:
		return "Weapon_MegaHogm.Drop";
		break;
...
//
// Input  : pObject - pointer to the object being thrown by the Hogm.
//-----------------------------------------------------------------------------
void CWeaponHogm::RecordThrownObject( CBaseEntity *pObject )
{
	thrown_objects_t2 thrown;
...
//
//-----------------------------------------------------------------------------
#define Hogm_THROWN_LIST_TIMEOUT	10.0f
void CWeaponHogm::PurgeThrownObjects()
{
	bool bListChanged;
	...
	do
	{
		...
		for( int i = 0 ; i < count ; i++ )
		{
			...
			else if( gpGlobals->curtime > (m_ThrownEntities[i].fTimeThrown + Hogm_THROWN_LIST_TIMEOUT) )
			{
				bRemove = true;
			}
			else
...
bool CWeaponHogm::IsAccountableForObject( CBaseEntity *pObject )
{
...
{
	CWeaponHogm *pCannon = dynamic_cast<CWeaponHogm *>(pActiveWeapon);
	if ( pCannon )
	{
...
{
	CWeaponHogm *pWeaponHogm = assert_cast<	CWeaponHogm* >( pAnim );
	pWeaponHogm->BeginUpgrade();
}


bool PlayerPickupControllerIsHoldingEntity2( CBaseEntity *pPickupControllerEntity, CBaseEntity *pHeldEntity )
{
	CPlayerPickupController2 *pController = dynamic_cast<CPlayerPickupController2 *>(pPickupControllerEntity);
...
float HogmGetHeldObjectMass( CBaseCombatWeapon *pActiveWeapon, IPhysicsObject *pHeldObject )
{
	...
	if ( pCannon )
	{
		CGrabController2 &grab = pCannon->GetGrabController();
...
CBaseEntity *GetPlayerHeldEntity2( CBasePlayer *pPlayer )
{
	...
	CPlayerPickupController2 *pPlayerPickupController = (CPlayerPickupController2 *)(pPlayer->GetUseEntity());
...
{
	CWeaponHogm *pCannon = dynamic_cast<CWeaponHogm *>(pHogm);
	if ( pCannon )
	{
...
float PlayerPickupGetHeldObjectMass2( CBaseEntity *pPickupControllerEntity, IPhysicsObject *pHeldObject )
{
	...
	if ( pController )
	{
		CGrabController2 &grab = pController->GetGrabController();
		mass = grab.GetSavedMass( pHeldObject );
	}
	return mass;
}
