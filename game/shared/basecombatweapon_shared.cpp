...
/* ironsight kickback option
ConVar vm_allowkickback("vm_allowkickback", "0", FCVAR_REPLICATED, "toggle ironsighted kickback", vm_allowkickback_callback );
void vm_allowkickback_callback( IConVar *pConVar, char const *pOldString, float flOldValue )
{
	if(m_bAllowKickBack){
		m_bAllowKickBack = false;
	} else {
		m_bAllowKickBack = true;
	}
}
*/
// ironsights
...
/* ironsight kickback option
static ConCommand toggle_ironsightkickback("toggle_ironsightkickback", CBaseCombatWeapon::ToggleKickBackAllowed);

void CC_ToggleIronSightKickBack( void )
{
	...
	engine->ServerCommand( "toggle_ironsightkickback" ); //forward to server
	//engine->ServerCmd( "toggle_ironsightkickback" ); //forward to server
}

bool CBaseCombatWeapon::IsKickBackAllowed( void ){
	return m_bAllowKickBack;
}

void CBaseCombatWeapon::ToggleKickBackAllowed( void ){

	engine->ServerCmd( "toggle_ironsightkickback" );
...
// We are not using prediction in singleplayer
	...
	if( pOwner->IsSuitEquipped() ){
		pOwner->SetMaxSpeed( 190 );
	}else{
		pOwner->SetMaxSpeed( 150 );
	}
...
{ 
	...
	/* ICEMOD - getting in a vehicle causes some "visability issues".
	if(pSwitchingTo == pOwner->GetActiveWeapon()){
...
#if !defined(CLIENT_DLL)
			if( !IsX360() || !ClassMatches("weapon_crossbow") || !ClassMatches("weapon_ar2") )
#endif
			{
				bFired = true;
			}
