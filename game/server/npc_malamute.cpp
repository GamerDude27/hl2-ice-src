...
enum
{
	COND_MALAMUTE_NONE = LAST_SHARED_CONDITION,
	COND_MALAMUTE_SIT,
	COND_MALAMUTE_STAND,
...
enum
{
	SCHED_MALAMUTE_NONE = LAST_SHARED_SCHEDULE, //BaseClass::NEXT_SCHEDULE LAST_SHARED_SCHEDULE
	SCHED_MALAMUTE_SIT,
...
enum
{
	TASK_MALAMUTE_NONE = LAST_SHARED_TASK,
	TASK_MALAMUTE_STAND,
	TASK_MALAMUTE_SIT,
...
Activity CNPC_Malamute::NPC_TranslateActivity( Activity newActivity )
{
	...
	/*
	if( newActivity == ACT_MALAMUTE_PICKUPITEM )
	{
		Msg("ACT PICKUPITEM\n");
		return ACT_MALAMUTE_PICKUPITEM;
	}
	else if( newActivity == ACT_MALAMUTE_DROPITEM )
	{
		Msg("ACT DROPITEM\n");
		return ACT_MALAMUTE_DROPITEM;
	}
	else
	{*/
		if (m_bExcited)
...
void CNPC_Malamute::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_MALAMUTE_PICKUPITEM )
	{
		...
		if ( pPlayer != NULL )
		{
			...
			{
				Msg("PICKUPOBJECT FAILD: GetTarget() != NULL && GetTarget() != pPlayer\n");
			}
		}	
	}
	...
	{
		// drop obect
		Msg(" o0 mal DROP ball Start OVER! 0o \n");
		m_iPO_State = POS_STATIC;
		//
		CBasePlayer *pPlayer = UTIL_PlayerByIndex( 1 );
		if ( pPlayer != NULL )
		{
			if(GetTarget() != NULL && GetTarget() != pPlayer)
			{
				...
				if( GetAttachment( "itempos", vecItemMouthPos ) )
				{
					...
					{
						Warning( "AE_MALAMUTE_DROPITEM:   m_hPhysicsEnt->VPhysicsGetObject == NULL!\n" );
					}
				}
			}
			else
			{
				Msg("DROPOBJECT FAILD: GetTarget() != NULL && GetTarget() != pPlayer\n");
			}
		}
	}
...
int CNPC_Malamute::SelectSchedule()
{
	...
	{
		Msg("/n SCHED_MALAMUTE_WATCHOBJECT \n");
		...
		else
		{
			...
			else if( HasCondition(COND_MALAMUTE_CLOSEPLAYER) )
			{
				if( HasCondition(COND_SEE_PLAYER) )
				{
					...
					if(testbark <= 1 )
					{
						Msg("CLOSEPLAYER shd SCHED_MALAMUTE_BARK\n");
						return SCHED_MALAMUTE_BARK;
					}
					else if(testbark >= 2 )
					{
						Msg(" SCHED_IDLE_STAND \n");
						//SetWait(1);
	...
	else
	{
		...
		{
			Msg("COND_LOST_PLAYER  SCHED_MALAMUTE_WANDER\n");
...
void CNPC_Malamute::GatherConditions( void )
{
	...
	else
	{
		...
		{
			// bool  a whistle to dog sound, once every time this is called.
			Msg("\nPast dist limit.%d.\n", m_iMaxWander);
			SetCondition(COND_MALAMUTE_CALLED_HOME);
		}
	}
	...
	if ( pPlayer != NULL )
	{
		...
		if( pObject )
		{
			...
			// if the player is close, get excited and want to play
		...
		if( m_iPO_State == POS_TOSSED )
		{
			...
			{
				Msg(" ball went static in .gather conditions. \n");
				m_iPO_State = POS_STATIC;
				SetTarget(pPlayer);
				TaskComplete();
			}
	...
	case TASK_MALAMUTE_SET_ITEM_GOAL:
		{
			...
			if( tr.fraction < 1.0 && tr.m_pEnt )
			{
				...
				Msg("TASK_MALAMUTE_SET_ITEM_GOAL\n");
				TaskComplete();
			}
			else
			{
					...
					if(GetTarget() != NULL && GetTarget() != pPlayer) // jason
					{
						...
						if( pObjectnew )
						{
							...
							if((m_flObjectDistance > BALL_MIN_TOSS_DISTANCE) && (f_curMass == MALAMUTE_FETCH_WEIGHT))
							{
								...
								Msg("light object, object weighs %f \n", f_curMass );
								Msg("Goood fetch distance, go get it! ");
								SetCondition( COND_MALAMUTE_FETCH ); //
								TaskComplete();
							}
						}
	...
	case TASK_WAIT_FOR_MOVEMENT:
		{
			...
			if( IsCurSchedule( SCHED_MALAMUTE_GRAB_RETURN ) )
			{
				Msg( "SCHED_MALAMUTE_GRAB_RETURN complete\n");
				TaskComplete();
				break;
			}
			if( IsCurSchedule( SCHED_MALAMUTE_DROPOBJECT ) )
			{
				Msg( "SCHED_MALAMUTE_DROPOBJECT complete\n");
				TaskComplete();
				break;
			}
			if( IsCurSchedule( SCHED_MALAMUTE_RETURN_TO_ORIGIN ) )
			{
				Msg("I made it home safe, lost excitement\n");
				m_bExcited = false;
			...
			/*
			if( IsCurSchedule( 	SCHED_IDLE_STAND || SCHED_IDLE_WALK || SCHED_IDLE_WANDER ) ) //&& m_bSitting == false
			{
				if(HasCondition( COND_SEE_PLAYER ))
				{
					TaskComplete();
...
{
	/*if( failedSchedule == SCHED_MALAMUTE_BACK_AWAY_FROM_ENEMY )
	{
		if( HasCondition( COND_CAN_RANGE_ATTACK1 ) )
		{
			// Most likely backed into a corner. Just blaze away.
			return SCHED_MALAMUTE_RANGE_ATTACK1;
		}
	}*/
