...
// ** ICE MOD NEW CODE BEGIN **
// Convars for camera bob
...
ConVar cl_viewbob_scale		( "cl_viewbob_scale", "0.03", 0, "Magnitude of Oscillation");
// ** ICE MOD NEW CODE END **
...
void CGameMovement::PlantFootprint( surfacedata_t *psurface )
{
	...
	if ( psurface->game.material != 'X' )//psurface->gameMaterial != 'X' )
	{
		...
		switch(psurface->game.material)//psurface->gameMaterial)
		{
		case 'D':
		...
		{
			Vector right;
			AngleVectors( pplayer->angles, 0, &right, 0 );//pev->angles, 0, &right, 0 );
			...
			// Splat a decal
			int index = decalsystem->GetDecalIndexForName( "FootPrint" ); // my line
...
void CGameMovement::WalkMove( void )
{
	...
	*/
	...
	{
		float testAgainstVel = player->GetAbsVelocity().Length();;
		Vector camoffset = 0;
		if(testAgainstVel <= 200){
			// walking
			camoffset.x = 0;
			camoffset.y = 0;
			camoffset.z = sin( gpGlobals->curtime * cl_viewbob_timer.GetFloat() ) * player->GetAbsVelocity().Length() * cl_viewbob_scale.GetFloat() / 200;
		}else{
			// running
			...
			camoffset.x = sin( gpGlobals->curtime * cl_viewbob_timer.GetFloat() ) * player->GetAbsVelocity().Length() * cl_viewbob_scale.GetFloat() / 800;
			camoffset.y = sin( gpGlobals->curtime * cl_viewbob_timer.GetFloat() ) * player->GetAbsVelocity().Length() * cl_viewbob_scale.GetFloat() / 300;
			camoffset.z = sin( gpGlobals->curtime * cl_viewbob_timer.GetFloat() ) * player->GetAbsVelocity().Length() * cl_viewbob_scale.GetFloat() / 100;
		}
		player->ViewPunch( QAngle( camoffset.x, camoffset.y, camoffset.z));
	}
	// ** ICE MOD CODE END **
	int i;
