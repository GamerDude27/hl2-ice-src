...
void CC_ToggleHelmet( void ) // called by binded key
{
	...
	//engine->ServerCommand( "toggle_helmet" ); //forward to server NOT SURE IF IT WILL WORK
}
 
static ConCommand toggle_helmet("toggle_helmet", CC_ToggleHelmet);
...
BEGIN_DATADESC( CBasePlayer )
	...
	DEFINE_FIELD( m_bPlayerWearingHelmet, FIELD_BOOLEAN ), // ICEMOD, save player helmet info
	...
	// helmet hint
	DEFINE_FIELD( m_iNumberOfHelmetEntries, FIELD_INTEGER ),
	DEFINE_FIELD( m_iNonSuppressedDamage,	FIELD_INTEGER ),
...
//	DEFINE_CUSTOM_FIELD( m_Activity, ActivityDataOps() ),
	...
	// ICEMOD footprint
	//DEFINE_FIELD( m_flStepSoundTime, FIELD_BOOLEAN ),
	//m_IsFootprintOnLeft - might not need to save this
...
CBasePlayer::CBasePlayer( )
{
	...
	//m_bPlayerWearingHelmet = false; // ICEMOD start helmet off
	// Helmet Hints
	m_bDidFreezHelmetHint = false;
	m_iNumberOfHelmetEntries = 0;
	m_iNonSuppressedDamage = 0;
...
{
	Msg("Update on Remove");
	SetWearingHelmet( false );
	VPhysicsDestroyObject();
...
int CBasePlayer::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	// ICEMOD Helmet lessens majority of damage types, and all of Freeze
	if ( m_Local.m_bWearingHelmet )
	{
		if(!(info.GetDamageType() & (DMG_FALL | DMG_DROWN )))
		{
			float flNewDMG = info.GetDamage() * .8f;
			info.SetDamage( flNewDMG );
		}

		if (bitsDamage & 16) //16 FREEZE bits hack
		{
			float flNewDMG = 0;
			info.SetDamage( flNewDMG );
		}
		
		if (bitsDamage & 4194304) //4194304 SLOW FREEZE bits hack
		{
			float flNewDMG = 0;
			info.SetDamage( flNewDMG );
		}
	...
	{
		// ICEMOD TODO: JASON - maybe add number of hits taken without helmet, to remind player they can wear a helmet!
		
		// Hint for Helmet if taking freeze damage
		if (bitsDamage & 16 || bitsDamage & 4194304) //16 FREEZE, 4194304 SLOW FREEZE
		{
			// m_bDidFreezHelmetHint is called once per life
			if(!m_bDidFreezHelmetHint)
			{
				UTIL_HudHintText( this, "#Valve_Hint_Helmet" );
				m_bDidFreezHelmetHint = true;
			}
		}

		// Hint for taking lots of damage without a helmet on over time, more than 
		// your regular health, and will only give this warning a few times.
		if ( (m_iNumberOfHelmetEntries < 3) && (m_iNonSuppressedDamage > 120) )
		{
			UTIL_HudHintText( this, "#Valve_Hint_Helmet" );
			m_iNonSuppressedDamage = 0;
			m_iNumberOfHelmetEntries++;
		}

		m_iNonSuppressedDamage += info.GetDamage();
	}
...
void CBasePlayer::OnDamagedByExplosion( const CTakeDamageInfo &info )
{
	...
	// Add new damage alpha to the helmet
	if(IsHelmetEquipped())
	{
		int curDmg = GetHelmetDamageAlpha();
		int newDmg = (int)distanceFromPlayer;
		if (newDmg > 255)
		{
			newDmg = 255;
		}
		...
		SetHelmetDamageAlpha(newDmg);

		// Jason? dosnt add gib squirts to the helmet.. :( erase?
		if ( info.GetDamageType() & DMG_ACID )
		{
			SetHelmetBloodColor(1);
			SetHelmetBloodAlpha(4);
		}
	}
}
...
{
	//if(IsHelmetEquipped()) // Jason - did not woork
		//RemoveHelmet();
...
bool CBasePlayer::GetInVehicle( IServerVehicle *pVehicle, int nRole )
{
	...
	if ( pVehicle->IsPassengerUsingStandardWeapons( nRole ) == false )
	{
		...
		if ( pWeapon != NULL )
		{
			// ICEMOD - make sure we are not ironsighted with the AR2, or it will crash HL2.
			if( pWeapon->m_bIsIronsighted )
				return false;

			// ICEMOD - changed from passing in NULL to pWeapon, to fix snowmobile glitches
...
void CBasePlayer::LeaveVehicle( const Vector &vecExitPoint, const QAngle &vecExitAngles )
{
	...
	if ( IsAlive() )
	{
		...
		{
			// This got fixed in basecombatweapon:

			/* ICEMOD - glitch getting in the vehicle may turn the weapon BLACK! dunno why
			//		but here is a hacky way to always return to the proper colors.
			CBaseCombatWeapon *pWeapon = GetActiveWeapon();
			if ( pWeapon != NULL )
			{
				Msg("Returned Color\n");
				//color32 color = pWeapon->GetRenderColor();
...
static void CreateJalopy( CBasePlayer *pPlayer )
{
	...
	// it won't, it will creat a snowmobile too.
	Vector vecForward;
	AngleVectors( pPlayer->EyeAngles(), &vecForward );
	CBaseEntity *pJeep = (CBaseEntity *)CreateEntityByName( "prop_vehicle_lynx" );
	if ( pJeep )
	{
		...
		pJeep->KeyValue( "model", "models/vehicles/snowmobile.mdl" );
		pJeep->KeyValue( "solid", "6" );
		pJeep->KeyValue( "targetname", "jeep" );
		pJeep->KeyValue( "vehiclescript", "scripts/vehicles/snowmobile.txt" );
		DispatchSpawn( pJeep );
		pJeep->Activate();
...
// Purpose: 
//-----------------------------------------------------------------------------
static void CreateLynx( CBasePlayer *pPlayer )
{
	// Cheat to create a snowmobile in front of the player
	Vector vecForward;
	AngleVectors( pPlayer->EyeAngles(), &vecForward );
//Tony; in sp sdk, we have prop_vehicle_hl2buggy; because episode 2 modified the jeep code to turn it into the jalopy instead of the regular buggy
	// well guy, I just changed it to a snowmobile!
#if defined ( SP_SDK )
	CBaseEntity *pJeep = (CBaseEntity *)CreateEntityByName( "prop_vehicle_lynx" );
#else
	CBaseEntity *pJeep = (CBaseEntity *)CreateEntityByName( "prop_vehicle_lynx" );
...
static ConCommand ch_createlynx("ch_createlynx", CC_CH_CreateLynx, "Spawn snowmobile in front of the player.", FCVAR_CHEAT);
...
void CBasePlayer::CheatImpulseCommands( int iImpulse )
{
	...
	case 101:
		...
		// ICE AMMOS
		GiveAmmo( 160,	"AtomSmasher" );
		GiveAmmo( 3,	"MobileTurretAmmo" );
		...
		// ICE Weapons
		GiveNamedItem( "weapon_apershotgun" );
		GiveNamedItem( "weapon_as_12" );
		GiveNamedItem( "weapon_machpistol" );
		GiveNamedItem( "weapon_machpistols" );
		GiveNamedItem( "weapon_atomsmasher" );
		GiveNamedItem( "weapon_mobileturret" );
		GiveNamedItem( "weapon_dsmg1" );
...
bool CBasePlayer::ClientCommand( const CCommand &args )
{
	...
	else if( stricmp( cmd, "toggle_helmet" ) == 0 ) // called by text
	{
		if(IsSuitEquipped())
		{

			Msg("stricmp( cmd.... called\n");

			if(m_bPlayerWearingHelmet)
			{
				RemoveHelmet();
			}
			else
			...
			//TogglePlayerHelmet();
			//engine->ServerCommand( "toggle_helmet" ); //forward to server
		}
		return true;
	}
	/* ICE MOD option for ironsighted kickback
	else if( stricmp( cmd, "toggle_ironsightkickback" ) == 0 )
	{
		CBaseCombatWeapon *pWeapon = GetActiveWeapon();
		if( pWeapon != NULL )
			pWeapon->ToggleKickBackAllowed();
...
bool CBasePlayer::BumpWeapon( CBaseCombatWeapon *pWeapon )
{
	...
	// ICEMOD - check to see if this is the machpistol and if we have it already, rename the weapon class to the dual machpistols class name
	if (Weapon_OwnsThisType( pWeapon->GetClassname(), pWeapon->GetSubType())) 
	{
		const char* T1wepName = pWeapon->GetClassname();
		//const char* T2wepName = "weapon_machpistol";

		//const char* myWeaponType = pPlayer->GetActiveWeapon()->GetClassname();
		if(FStrEq(T1wepName,"weapon_machpistol"))
		{
			//Warning ( "You own a mach pistol already, give machpistols /n");

			//It is another mach_pistol so rename the class
			//const char* T3Duel = "weapon_machpistols";
			//pWeapon->SetClassname(T3Duel);
			//pWeapon->

			//Warning (" new class name /n");
			//Warning (pWeapon->GetClassname());


			//hack!!
			GiveNamedItem( "weapon_machpistols" );
		} else if(FStrEq(T1wepName, "weapon_smg1"))
		{
			GiveNamedItem( "weapon_dsmg1" );
		}
	}
	...
	if (Weapon_OwnsThisType( pWeapon->GetClassname(), pWeapon->GetSubType())) 
	{
		/* ICE - check to see if it is another mach_pistol and we dont already have the duels
		const char* T1wepName = pWeapon->GetClassname();
		const char* T2wepName = "weapon_machpistol";
		...
		if(*T1wepName == *T2wepName){
			//Warning( "This code works \n");
			//It is another mach_pistol so try to give you the duels
			const char* T3Duel = "weapon_machpistols";
			pWeapon->SetClassname(T3Duel);
		}
		*/
...
void CBasePlayer::UpdateClientData( void )
{
	...
	m_Local.m_bPoisoned = ( m_bitsDamageType & DMG_POISON ) 
						...
						&& ( m_iHealth < 100 );
	// ICEMOD
	// helmet effects update
	//
	if( IsHelmetEquipped() )
	{
		// update helmet blast damage effect
		if ( gpGlobals->curtime > m_flHelmetDamageAlphaTime)
		{
			if( m_Local.m_iHelmetDamageAlpha >= 1)
			{
				//Msg("lowerd damage alpha\n");
				m_Local.m_iHelmetDamageAlpha--;
			}
			m_flHelmetDamageAlphaTime = gpGlobals->curtime + .05f;
		}
		// update helmet blood effects
		if ( gpGlobals->curtime > m_flHelmetBloodAlphaTime)
		{
			if( m_Local.m_iHelmetBloodAlpha >= 1)
			{
				Msg("lowerd damage alpha\n");
				m_Local.m_iHelmetBloodAlpha--;
			}
			m_flHelmetBloodAlphaTime = gpGlobals->curtime + 0.5f; //m_flHelmetBloodAlphaTime = gpGlobals->curtime + .05f;
		}
	}
	else
	{
		// set helmet blast damage effect 0
		if( m_Local.m_iHelmetDamageAlpha != 0)
		{
			m_Local.m_iHelmetDamageAlpha = 0;
		}
		// set helmet blood effect 0 and color
		if( m_Local.m_iHelmetBloodAlpha != 0)
		{
			m_Local.m_iHelmetBloodAlpha = 0;
			m_Local.m_iHelmetBloodColor = 0;
		}
	}
...
void CStripWeapons::StripWeapons(inputdata_t &data, bool stripSuit)
{
	...
	{
		if( stripSuit )
			pPlayer->SetWearingHelmet( false );
...
void CBasePlayer::EquipHelmet( void )
{ 
	//if(m_Local.m_bWearingHelmet != true)
	//{
		CBasePlayer *pPlayer = UTIL_PlayerByIndex(1);
		...
		else
		{
			...
			{
				// Do a viewpunch downward
				pPlayer->ViewPunch( QAngle( 20, 0, 0 ) );
				
				// Fade in From Black
				color32 black = {32,32,32,255}; //{32,32,32,200}
				UTIL_ScreenFade( pPlayer, black, 0.4, 0, FFADE_IN  );//( pPlayer, black, 0.4, 0, FFADE_IN  )

				// play sound
				EmitSound( "Player.PickupWeapon" ); //Player.PickupWeapon
				...
				if(pPlayer->HasWeapons())
				{
					if ( !IsInAVehicle() )
					{
						...
						if(pActiveWeapon->IsIronsighted())
							pActiveWeapon->ToggleIronsights();

						// interrupt shooting & re-deploy weapon
						pActiveWeapon->Deploy();
					
						// envelope sound
						// replay draw animation
						//pActiveWeapon->SendWeaponAnim( ACT_VM_DRAW );
					}
				}

				// draw helmet
				m_Local.m_bWearingHelmet = true;
				m_bPlayerWearingHelmet = true;

				// Send a message to hide the scope
				CSingleUserRecipientFilter filter(pPlayer);
				UserMessageBegin(filter, "ShowHelmet");
					WRITE_BYTE(1);
				MessageEnd();
			}
		}
...
void CBasePlayer::SetWearingHelmet( void )
{
	// used to setup the helmet when loading a game
	if(m_bPlayerWearingHelmet)
	{
		EquipHelmet();
	}
...
void CBasePlayer::SetWearingHelmet( bool option )
{
	// used to setup the helmet when loading a game
	if(option)
	{
...
void CBasePlayer::RemoveHelmet( void )
{
	CBasePlayer *pPlayer = UTIL_PlayerByIndex(1);
	...
	else
	{
		if(pPlayer->IsSuitEquipped())
		{
			// Do a viewpunch upward
			pPlayer->ViewPunch( QAngle( -20, 0, 0 ) );
				
			// Fade in From Black
			//color32 black = {255,255,255,200};
			//UTIL_ScreenFade( pPlayer, black, 0.4, 0, FFADE_IN  );
			...
			// remove helmit
			m_Local.m_bWearingHelmet = false;
			m_bPlayerWearingHelmet = false;
...
{ 
	...
	// ICEMOD Return a hud hint for the helmet
	CBasePlayer *pPlayer = UTIL_PlayerByIndex(1);
	...
	CFmtStr hint;
	hint.sprintf( "#Valve_Hint_Helmet" );
	UTIL_HudHintText( this, hint.Access() );
}
...
{
	SetWearingHelmet( false );
	//RemoveHelmet(); // when you die, your suit is removed so should your helmet?
