...
void CWeaponAR2::ItemPostFrame( void )
{
	...
	// ICE MOD, check if play is ironsight, and restric running
	CBaseCombatWeapon *pWeapon = pOwner->GetActiveWeapon();
	if( pWeapon->m_bIsIronsighted ){
		if( pOwner->IsSuitEquipped() )
		{
			pOwner->SetMaxSpeed( m_pWalkSuit );
		}
		else
		{
			pOwner->SetMaxSpeed( m_pWalkNorm );
		}
	}
...
Activity CWeaponAR2::GetPrimaryAttackActivity( void )
{
	...
	if( pWeapon->m_bIsIronsighted ){
		// better accuracy, no kickback if in ironsight
	...
	/*	chenged from ACT_VM_RECOIL3, which is now our "steady fire" animation
		for people who don't like the kickback in ironsighted mode.. 
		This convar option is in basecombatweapon_shared, and is called ironsight_kickback
	*/
	return ACT_VM_RECOIL2;
}
...
void CWeaponAR2::DelayedAttack( void )
{
	...
	/* REMOVED FOR OUR NEW WEAPON IN ICE MOD THAT DONT use this
...
void CWeaponAR2::AddViewKick( void )
{
	...
	if( !pWeapon->m_bIsIronsighted ){
		// not in iron sight dont have kick 
		...
		// in iron sight, but keep a little kick or it feels flat
		float flDuration = m_fFireDuration / 2.5;
