...
BEGIN_DATADESC( CNPC_BaseZombie )
	...
	DEFINE_FIELD( m_fIsTorso, FIELD_INTEGER ),
	DEFINE_FIELD( m_fModelHalfWidth, FIELD_INTEGER ),
	/* 
		ICE 
		m_fIsTorso was changed from FIELD_BOOLEAN to FIELD_INTEGER, so we can have a few new NPC types that behave
		like the zombie or zombie_torso. So now m_fIsTorso is setup like this:
		...
		This also lets us change some of the code and keep the regular zombies in the game, for example
		penguins do not drop headcrabs when they die.
	*/
...
CNPC_BaseZombie::CNPC_BaseZombie()
{
	...
	m_fModelHalfWidth = FOOTPRINT_ZOMBIE;
...
{
	if( m_fIsTorso == 1 || m_fIsTorso == 2 || m_fIsTorso == 3) // Torso, Penguin, Baby Penguin
	{
		return( 60 );
	}
...
HeadcrabRelease_t CNPC_BaseZombie::ShouldReleaseHeadcrab( const CTakeDamageInfo &info, float flDamageThreshold )
{
	if ( m_iHealth <= 0 )
	{
		...
		// ICE, not sure if the above code needs change to release the head crab
		if ((m_fIsTorso != 0) && IsChopped( info ) ) // ICE just added the != 0 for torso, and penguins
		{
...
int CNPC_BaseZombie::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	if( (m_fIsTorso == 0) && (bChopped || bSquashed) && !bKilledByVehicle && !(info.GetDamageType() & DMG_REMOVENORAGDOLL) ) // !m_fIsTorso changed to "== 0"
	{
		if( bChopped )
		{
	...
	else if (m_fIsTorso == 1) // ICE, added if (m_fIsTorso == 1), because the others dont release headcrabs 
	{
...
void CNPC_BaseZombie::DieChopped( const CTakeDamageInfo &info )
{
	...
	if ( UTIL_ShouldShowBlood( BLOOD_COLOR_RED ) ) // ICE YELLOW to RED
	{
		int i;
		Vector vecSpot;
		Vector vecDir;
		...
		for ( i = 0 ; i < 4; i++ )
		{
			...
			UTIL_BloodDrips( vecSpot, vec3_origin, BLOOD_COLOR_RED, 50 ); // ICE YELLOW to RED
		}
		...
		for ( int i = 0 ; i < 4 ; i++ )
		{
			...
			UTIL_BloodImpact( vecSpot, vecDir, BLOOD_COLOR_RED, 1 ); // ICE changed BloodColor() to BLOOD_COLOR_RED
		}
	}
}
...
CBaseEntity *CNPC_BaseZombie::ClawAttack( float flDist, int iDamage, QAngle &qaViewPunch, Vector &vecVelocityPunch, int BloodOrigin  )
{
	...
	if ( pHurt )
	{
		//if (m_fIsTorso <= 1)  ICE Enemies 1 and below, can hurt you.. 2 and 3, the penguins cannot hurt you. but they do an animation and can attack other things
		//{
			AttackHitSound();
...
void CNPC_BaseZombie::SnowFootPrint( bool IsLeft )
{
	trace_t tr;
	Vector traceStart;
	QAngle angles;
			...
			if ( psurf->game.material != 'X' )
			{
				...
				if (footprintDecal != -1)
				{
					...
					VectorMA( GetAbsOrigin(), 
						IsLeft ? -m_fModelHalfWidth : m_fModelHalfWidth,
						right, hipOrigin );
					...
					if(m_fIsTorso == 2)
					{
						fpindex = decalsystem->GetDecalIndexForName( "FootPrintPenguin" );
					}
					else if(m_fIsTorso == 3)
					{
						fpindex = decalsystem->GetDecalIndexForName( "FootPrintBabyPenguin" );
					} else 
					{
						fpindex = FootPrintDecal(); // get the decal by default
					}
...
void CNPC_BaseZombie::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_ZOMBIE_STEP_LEFT )
	{
		...
		FootstepSound( false );
		// ICEMOD FOOTPRINT NPC ZOMBIE BASE
		SnowFootPrint( true );
		return;
	}
	...
	if ( pEvent->event == AE_ZOMBIE_ATTACK_RIGHT )
	{
		...
		right = right * 100;
		forward = forward * 200;
		if(m_fIsTorso <= 1) // ICE, no let penguins hurt you
...
void CNPC_BaseZombie::Spawn( void )
{
	...
	// push penguins to floor, fixes anything?
	if(  m_fIsTorso == 2 || m_fIsTorso == 3 )
	{
		trace_t tr;
...
void CNPC_BaseZombie::Precache( void )
{
	...
	PrecacheParticleSystem( "blood_impact_red_01" ); // ICE changed from "blood_impact_zombie_01" to "blood_impact_red_01"
...
int	CNPC_BaseZombie::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
	...
	{
		if ( (m_fIsTorso == 0) && IsPathTaskFailure( taskFailCode ) && // !m_fIsTorso change to == 0
...
void CNPC_BaseZombie::GatherConditions( void )
{
	...
	if( m_NPCState == NPC_STATE_COMBAT && (m_fIsTorso == 0) ) // !m_fIsTorso change to == 0
	{
...
{
	if( m_fIsTorso != 0 ) // changed to != 0, torso, and penguins
	{
	...
	if ( !m_fIsHeadless )
	{
		...
		m_fIsTorso = 1; // changed from true to 1
	...
	if ( m_fIsTorso == 1 || m_fIsTorso == 2 || m_fIsTorso == 3 ) // changed from true to 1 2 && 3 for now.. torso, penguin, baby penguin
	{
...
void CNPC_BaseZombie::Event_Killed( const CTakeDamageInfo &info )
{
	if ( info.GetDamageType() & DMG_VEHICLE )
	{
		...
		// Big blood splat
		UTIL_BloodSpray( WorldSpaceCenter(), vecDamageDir, BLOOD_COLOR_RED, 8, FX_BLOODSPRAY_CLOUD ); // ICE BLOOD_COLOR_YELLOW, changed to RED
	}
...
{
	bool bKilledByVehicle = ( ( info.GetDamageType() & DMG_VEHICLE ) != 0 );
	if((m_fIsTorso != 0) || (!IsChopped(info) && !IsSquashed(info)) || bKilledByVehicle ) // change to !=0 for torso, and penguins
	{
		return BaseClass::BecomeRagdoll( info, forceVector );
	}
	
	// do same for m_fIsTorso == 2 and 3
	if((m_fIsTorso == 2) || (m_fIsTorso == 3))
	{
		return BaseClass::BecomeRagdoll( info, forceVector );
	}
...
void CNPC_BaseZombie::ReleaseHeadcrab( const Vector &vecOrigin, const Vector &vecVelocity, bool fRemoveHead, bool fRagdollBody, bool fRagdollCrab )
{
	...
	if( m_fIsTorso == 0 ) // change to == 0 from !
	{
		vecSpot.z -= 16;
	}
	...
	if( fRagdollCrab )
	{
		...
		if ( pGib )
		{
			...
			if( UTIL_ShouldShowBlood(BLOOD_COLOR_RED) ) // ICEmod YELLOW to RED
			{
				UTIL_BloodImpact( pGib->WorldSpaceCenter(), Vector(0,0,1), BLOOD_COLOR_RED, 1 ); // ICEmod YELLOW to RED
				...
				for ( int i = 0 ; i < 3 ; i++ )
				{
					...
					UTIL_BloodDrips( vecSpot, vec3_origin, BLOOD_COLOR_RED, 50 ); // ICEmod YELLOW to RED
				}
			}
		}
	}
...
{
	if( m_fIsTorso == 1 || m_fIsTorso == 2 || m_fIsTorso == 3) // changed from "true"
	{
		return 12.0f;
	}
